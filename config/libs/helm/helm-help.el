...
**** Use `\\<helm-ff-map>\\[helm-execute-persistent-action]' (persistent action) on a directory to go down one level

On a symlinked directory a prefix argument expands to its true name.

**** Use `\\<helm-ff-map>\\[helm-ff-up-one-level]' or `DEL' on a directory to go up one level
...
**** Use `\\<helm-ff-map>\\[helm-ff-down-last-level]' to walk back the resulting tree of all the `\\<helm-ff-map>\\[helm-ff-up-one-level]' or DEL you did
...
Note that when copying, renaming, etc. from `helm-ff' the
...
\\<helm-ff-map>\\[helm-ff-toggle-dirs-only] and \\<helm-ff-map>\\[helm-ff-toggle-files-only].
...
\\<helm-ff-map>\\[helm-ff-sort-alpha], \\[helm-ff-sort-by-newest] or \\[helm-ff-sort-by-size].
NOTE:
...
This is different from using `\\<helm-ff-map>\\[helm-ff-up-one-level]' in that it moves
...
*** helm-ff supports fuzzy matching
...
TIP: Use `\\<helm-map>\\[helm-toggle-resplit-and-swap-windows]' to display Helm window vertically
...
toggle the thumbnail view with \\<helm-ff-map>`\\[helm-ff-toggle-thumbnails]'.

**** Launch a slideshow from marked files

Helm provides an action from `helm-ff' that allows
...
- Open file with external program (`\\<helm-ff-map>\\[helm-ff-run-open-file-externally]',`C-u' to choose).
...
version control.  Otherwise it falls back to helm-ff.  See
...
Regardless Helm uses the basename of the pattern entered in the helm-ff
...
:default (thing-at-point 'symbol) :use-default-as-input t
to `helm-source-locate' helm call.
...
by entering \"**.JPG\" at the end of the helm-ff pattern, then hit
\\<helm-ff-map>\\[helm-ff-run-query-replace-fnames-on-marked] and enter \"JPG\" on first prompt, then \"jpg\" on second prompt and hit `RET'.
...
You can open a dired buffer containing only marked files with `\\<helm-ff-map>\\[helm-ff-run-marked-files-in-dired]'.
...
The command \\<helm-ff-map>\\[helm-ff-run-mcp] allows
...
3) From helm-ff use adb tramp method:
    /adb::/
From there you can navigate as usual, mark and copy files etc...

*** Bookmark the `helm-ff' session

You can bookmark the `helm-ff' session with `\\[helm-ff-bookmark-set]'.
You can later retrieve these bookmarks by calling `helm-filtered-bookmarks'
or, from the current `helm-ff' session, by hitting `\\[helm-ff-switch-to-bookmark]'.

*** Grep files from `helm-ff'

You can grep individual files from `helm-ff' by using
\`\\<helm-ff-map>\\[helm-ff-run-grep]'.  This same command can also
...
or archives, use zgrep with \`\\<helm-ff-map>\\[helm-ff-run-zgrep]'.

Otherwise you can use recursive commands like \`\\<helm-ff-map>\\[helm-ff-run-grep-ag]' or `\\<helm-ff-map>\\[helm-ff-run-git-grep]'
that are much faster than using `\\<helm-ff-map>\\[helm-ff-run-grep]' with a prefix argument.
See `helm-grep-ag-command' and `helm-grep-git-grep-command' to set this up.

You can also use \"id-utils\"' GID with \`\\<helm-ff-map>\\[helm-ff-run-gid]'
...
commands not available in `helm-ff' actions and use them
from `\\<helm-ff-map>\\[helm-ff-run-eshell-command-on-file]'.

Example: You want a command to uncompress some \"*.tar.gz\" files from `helm-ff':

1) Create an Eshell alias named, say, \"untargz\" with the command
\"alias untargz tar zxvf $*\".

2) Now from `helm-ff' select the \"*.tar.gz\" file (you can also
mark files if needed) and hit `\\<helm-ff-map>\\[helm-ff-run-eshell-command-on-file]'.
...
see the specific info page once you hit `\\<helm-ff-map>\\[helm-ff-run-eshell-command-on-file]'.

*** Using TRAMP with `helm-ff' to read remote directories

`helm-ff' works fine with TRAMP despite some limitations.
...
in action menu, otherwise it is available at any time with \\<helm-ff-map>\\[helm-ff-run-mail-attach-files].
...
displayed.  When using \\<helm-ff-map>\\[helm-ff-run-switch-other-window] the current
...
When running `\\<helm-ff-map>\\[helm-ff-run-chmod]' on
...
NOTE: Another way to change modes on files in helm-ff is
running `\\<helm-ff-map>\\[helm-ff-run-switch-to-shell]' and use 'chmod' directly.

*** Delete files

You can delete files without quitting helm with
`\\<helm-ff-map>\\[helm-ff-persistent-delete]' or delete files and quit helm with `\\[helm-ff-run-delete-file]'.
...
Tip: Navigate to your Trash/files directories with `helm-ff' and set a bookmark
there with \\<helm-ff-map>\\[helm-ff-bookmark-set] for fast access to Trash.
...
helm-ff can ignore files matching
...
You may have in some rare cases to refresh directory manually with `\\<helm-map>\\[helm-force-update]'
for example when helm-ff session is running and a file is modified/deleted
...
you will have to refresh manually directories when needed with `\\<helm-map>\\[helm-force-update]'.
...
|\\[helm-ff-up-one-level]|Go to parent directory.
|\\[helm-ff-history]|Switch to the visited-directory history.
...
|\\[helm-ff-switch-to-bookmark]|Jump to bookmark list.
...
** Tips
You can open directly the selected file and exit helm or preselect the file in helm-ff,
...
(defun helm-read-file-name-help-message ()
	(let (
		(name (if helm-alive-p (helm-source-name (helm-get-current-source)) "generic"))
	)
		(format
...
Don't confuse this with `helm-ff' which is a native helm command,
...
This is different from using `\\[helm-ff-up-one-level]' in that it moves
the cursor to the top instead of remaining on the previous subdir name.
...
By default `helm-read-file-name' uses the persistent actions of `helm-ff'.
...
slower than using grep recusively (see helm-ff
...
accessible from `helm-ff' which allow you to navigate to another
...
As always you can access Helm AG from `helm-ff'.
...
directory in helm-ff.  If this current directory is a subdirectory of a
...
Start from `helm-buffers-list' or `helm-mini', mark some buffers and use 'helm-multi-occur-as-action'.
...
you can enable `helm-follow-mode' with `\\<helm-map>\\[helm-follow-mode]'.
...
You can refresh the buffer by running `\\<helm-map>\\[helm-force-update]'.
...
(defun helm-comp-read-help-message ()
	(let ((com (helm-source-name (helm-get-current-source))))
		(format
