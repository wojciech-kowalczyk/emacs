...
(define_face 'rxt-highlight-face '((t :background "#222222"))
...
Verbose primitives are things like `line-start' instead of `bol', etc.")
...
(cl-defun rxt-interactive/elisp (&optional (prompt "Emacs regexp: "))
	(list
		(cond
			(current-prefix-arg (read-string prompt))
...
(cl-defun rxt-interactive/pcre (&optional (prompt "PCRE regexp: "))
	(list
		(cond
			(current-prefix-arg (rxt--read-pcre prompt))
			(mark-active (buffer-substring-no-properties (region-beginning) (region-end)))
			(t
				(condition-case nil
...
(defun rxt--toggle-s-mode () "Toggle emulated PCRE single-line (s) flag."
	(interactive)
	(rxt--toggle-flag ?s))

(defun rxt--toggle-x-mode () "Toggle emulated PCRE extended (x) flag."
	(interactive)
	(rxt--toggle-flag ?x))

(defun rxt--toggle-i-mode () "Toggle emulated PCRE case-insensitive (i) flag."
	(interactive)
	(rxt--toggle-flag ?i))

(defun rxt--toggle-flag (char) "Toggle CHAR, a PCRE flag."
	(cond
		; RE-Builder
		((derived-mode-p 'reb-mode) (rxt--toggle-flag-re-builder char))
		((minibufferp) (rxt--toggle-flag-minibuffer char))
		(isearch-mode (rxt--toggle-flag-isearch char))
		(t (error "Not in minibuffer, RE-Builder or isearch mode."))))
...
(defun rxt--toggle-flag-minibuffer (char)
	...
	; That emited byte compile errors so I changed this.
	; Not exactly sure if this is the same though.
	(let ((str (rxt--toggle-flag-string (minibuffer-contents) char)))
		(goto-char (point-max))
...
(defun rxt--toggle-flag-isearch (char)
	(when isearch-regexp
		(setq isearch-string (rxt--toggle-flag-string isearch-string char))
		(setq isearch-message
			(mapconcat #'isearch-text-char-description isearch-string ""))
		(isearch-search-and-update)))

(defun rxt--toggle-flag-string (string char)
	(if (string-match (rx string-start "(?" (group (+ (any ?i ?s ?x))) ")") string)
...
(defun rxt--xor-flags (flags char)
	(concat (sort (cl-set-exclusive-or (string-to-list flags) (list char)) #'<)))

; Minor mode for using emulated PCRE syntax

(defvar-local pcre-old-isearch-search-fun-function nil
	...
	(if pcre-mode
		...
		(progn
			...
			(setq pcre-old-isearch-key-bindings
				(cl-loop
					for key being the key-seqs of rxt--read-pcre-mode-map
					for def = (lookup-key isearch-mode-map key)
					collect (cons (copy-sequence key) (if (numberp def) nil def))))
			(cl-loop
				for key being the key-seqs of rxt--read-pcre-mode-map
		...
		(cl-loop
			for (key . def) in pcre-old-isearch-key-bindings
			do (define-key isearch-mode-map key def)))
...
(defun pcre-to-elisp/cached (pcre) "Translate PCRE to Emacs syntax, caching both forms."
	(or
		(gethash pcre pcre-mode-cache)
...
"Add a PCRE-to-Emacs translation to the `pcre-mode' cache."
	(and
		(/= (length pcre-regexp) 0)
		(/= (length emacs-regexp) 0)
		...
		(if (= (ring-length pcre-cache-ring) (ring-size pcre-cache-ring))
			(let ((old-item (ring-remove pcre-cache-ring)))
				(remhash (car old-item) pcre-mode-cache)
				(remhash (cdr old-item) pcre-mode-reverse-cache))
...
(defun pcre-isearch-mode-hook ()
	(unless (eq isearch-search-fun-function #'isearch-search-fun-default)
	...
	; exited without restoring the original search function for some reason.
	...
	(setq-local isearch-search-fun-function #'pcre-isearch-search-fun-function))
	...
	(lambda (string bound noerror)
		(let (
			(real-search-function
				(funcall
					(or
						pcre-old-isearch-search-fun-function
...
"Add \"PCRE\" to the Isearch message when searching by regexp in `pcre-mode'."
	(and
		...
		(let ((message ad-return-value))
			...
			; the original
			(when (let ((case-fold-search t)) (string-match "regexp" message))
				(let* (
					(match (match-string 0 message))
...
;	(ad-set-arg 0 nil))
;
;(defadvice isearch-edit-string (around pcre-mode disable)
;"Add PCRE mode-toggling keys to Isearch minibuffer in regexp mode."
;	(if isearch-regexp
;		(minibuffer-with-setup-hook #'rxt--read-pcre-mode ad-do-it)
;		ad-do-it))


(defun pcre-decorate-search-function (real-search-function)
	`(lambda (string &optional bound noerror count)
		(,real-search-function (pcre-to-elisp/cached string) bound noerror count)))
...
;			(when original (ad-set-arg 1 original)))))
;
...
"Toggle the regexp near point between Elisp string and rx syntax."
	...
	(let* ((context (syntax-ppss)) (string-start (nth 8 context)))
		(cond
			(string-start (goto-char string-start))
	...
	(let* ((regex (read (current-buffer))) (print-escape-newlines t))
		(save-excursion
			(if (listp regex)
				...
				(save-restriction
					...
					(pp-buffer)
					; Eemove the extra newline that pp-buffer inserts.
					(goto-char (point-max))
					(delete-region
						(point)
...
(defun rxt--add-flags (pcre flags) "Prepend FLAGS to PCRE."
	(if (= (length flags) 0) pcre (concat "(?" flags ")" pcre)))
...
interactively."
	(interactive (rxt-interactive/elisp))
	(let ((rxt-explain t) (rxt-verbose-rx-translation rxt-explain-verbosely))
...
; Macro: interactively call one of two functions depending on the major mode.
...
(define-minor-mode rxt-mode "Regex translation utilities." :keymap rxt-mode-map)

(define-globalized-minor-mode rxt-global-mode rxt-mode #'rxt-mode)
...
(add-hook 'rxt-help-mode-hook
	(lambda () (and (boundp 'paredit-mode) paredit-mode (paredit-mode -1))))
...
"Display string regexp REGEXP with its `rx' form RX in an `rxt-help-mode' buffer."
	(with-current-buffer (get-buffer-create "* Regexp Explain *")
		(let ((print-escape-newlines t) (inhibit-read-only t))
			(erase-buffer)
			(rxt-help-mode)
...
string (see `rxt-explain-elisp', `rxt-explain-pcre' and `rxt--make-help-overlays')."
	(let ((start (point)))
		(cl-typecase rx
			(cons
				(pcase rx
					...
					(`(,(and (or `repeat `= `>=) head) ,(and (pred integerp) n) . ,rest)
						(insert (format "(%s %d" head n))
						(rxt--print-list-tail rest))
					(_ (rxt--print-list-tail rx t))))
			(symbol
				(cl-case rx
			...
			(string (insert (rxt--propertize-whitespace (prin1-to-string rx))))
			(character
				(cond
					((eq ?\s rx) (insert "?"))
					((memq rx rxt--print-special-chars) (insert "?\\" rx))
					((assq rx rxt--print-char-alist)
						(insert "?" (assoc-default rx rxt--print-char-alist)))
					(t (insert "?" (char-to-string rx)))))
			(t (prin1 rx (current-buffer))))
		(when rxt--print-with-overlays (rxt--make-help-overlays rx start (point)))))
...
(defun rxt--make-help-overlays (rx start end)
	(let ((location (rxt-location rx)))
		(and
			...
			(let* (
				(sexp-begin (copy-marker start t))
				...
				(sexp-ol (make-overlay sexp-begin sexp-end nil t))
				(source-ol (make-overlay source-begin source-end nil t))
			)
...
(defun rxt--propertize-whitespace (string)
	(let ((string (copy-sequence string)) (start 0))
...
(if (fboundp 'define-error)
	...
	(put 'rxt-invalid-regexp 'error-conditions '(rxt-invalid-regexp invalid-regexp error))
...
(defun rxt-location-text (location)
	(when (rxt-location-p location)
		(let (
			(start (rxt-location-start  location))
	...
	(when (consp res)
		(let (
			...
			(cond
				...
				; Drop trivial elts
				((and (rxt-string-p re) (equal (rxt-string-chars re) "")) tail)
				; Flatten strings
				((and (rxt-string-p re) (consp tail) (rxt-string-p (car tail)))
					(cons (rxt-string-concat re (car tail)) (cdr tail)))
				(t (cons re tail))))))

(defun rxt-string-concat (str1 str2)
	(if (eq (rxt-string-case-fold str1) (rxt-string-case-fold str2))
		(let (
			(result
				(rxt-string
					(concat
						(rxt-string-chars str1)
			...
			(and
				...
				(setf (rxt-location result)
					(make-rxt-location
						:source (rxt-location-source first)
...
(defun rxt-empty-p (re)
	(or (and (rxt-choice-p re) (null (rxt-choice-elts re))) (rxt-empty-char-set-p re)))
	...
	(cl-destructuring-bind (other-elements char-set case-fold-char-set)
		...
		(let (
			(simplified-alternatives
				(nconc
					(unless (rxt-empty-p char-set) (list char-set))
					(unless (rxt-empty-p case-fold-char-set) (list case-fold-char-set))
					other-elements))
		)
			(pcase simplified-alternatives
				(`() rxt-empty)
				(`(,element) element)
				(_ (make-rxt-choice simplified-alternatives))))))
	...
	(if alternatives
		(let ((re (car alternatives)))
			(cl-destructuring-bind (tail char-set case-fold-char-set)
				...
				(cond
					((rxt-choice-p re) ; Flatten nested choices.
						(list
					...
					((rxt-char-set-union-p re) ; Fold char sets together.
						(if (rxt-char-set-union-case-fold re)
							...
							(list
								...
								case-fold-char-set)))
					; Same for 1-char strings.
					((and (rxt-string-p re) (= 1 (length (rxt-string-chars re))))
					...
					(t (list (cons re tail) char-set case-fold-char-set)))))
		(list
			nil
...
; Repetition
(cl-defstruct (rxt-repeat (:include rxt-syntax-tree)) from to body greedy)

(cl-defun rxt-repeat (from to body &optional (greedy t))
	(if (equal to 0)
		(rxt-string "")
		(make-rxt-repeat :from from :to to :body body :greedy greedy)))
...
(cl-defstruct (rxt-syntax-class (:include rxt-syntax-tree)) symbol)
...
(cl-defstruct (rxt-char-category (:include rxt-syntax-tree)) symbol)
...
(cl-defstruct (rxt-char-set-union (:include rxt-char-set))
...
same `case-fold' property."
	(let ((case-fold 'undetermined) chars ranges classes)
		(dolist (item items)
			(cl-etypecase item
				(character (push item chars))
				...
				(rxt-char-set-union
					(if (eq case-fold 'undetermined)
						...
						(unless (eq case-fold (rxt-char-set-union-case-fold item))
							(error
								"Cannot construct union of char-sets with unlike case-fold setting: %S"
								item)))
		...
		(make-rxt-char-set-union
			:chars chars :ranges ranges :classes classes
			:case-fold (if (eq case-fold 'undetermined) rxt-pcre-case-fold case-fold))))
...
"Return a list of all characters in CHAR-SET."
	...
	(append
		...
		(mapcan
			(lambda (range)
				(cl-loop for char from (car range) to (cdr range) collect char))
			(rxt-char-set-union-ranges char-set))))
...
lowercase versions of all characters in CHAR-SET."
	(cl-assert (rxt-char-set-union-p char-set))
	(let* (
		(classes (rxt-char-set-union-classes char-set))
		(all-chars
			(if case-fold-p
				(mapcan
					(lambda (char) (list (upcase char) (downcase char)))
					(rxt--all-char-set-union-chars char-set))
				(rxt--all-char-set-union-chars char-set)))
		(all-ranges
			(rxt--extract-ranges (rxt--remove-redundant-chars all-chars classes)))
	)
		(let (singletons ranges)
			(cl-loop
				for (start . end) in all-ranges
				do
				(cond
					((= start end) (push start singletons))
					((= (1+ start) end)
						(push start singletons)
						(push end singletons))
					(t (push (cons start end) ranges))))
			(make-rxt-char-set-union
				:chars (nreverse singletons)
				:ranges (nreverse ranges)
				:classes classes
				:case-fold
					(unless case-fold-p (rxt-char-set-union-case-fold char-set))))))
...
Example:
    (rxt--extract-ranges (list ?a ?b ?c ?q ?x ?y ?z))
        => ((?a . ?c) (?q . ?q) (?x . ?z))"
	(let (
		(array (apply #'vector (cl-delete-duplicates (sort (copy-sequence chars) #'<))))
	)
		(cl-labels (
			(recur (start end)
				(unless (< end start)
					(let (
						(min (aref array start))
						(max (aref array end))
					)
						(if (= (- max min) (- end start))
							(list (cons min max))
							(let* (
								(split-point (/ (+ start end) 2))
			...
			(pcre2el-merge (left right)
				(cond
					...
					(t
						(let (
							(last-left (car (last left)))
							(first-right (car right))
						)
							(if (= (1+ (cdr last-left)) (car first-right))
								(append
									(cl-subseq left 0 -1)
									(list (cons (car last-left) (cdr first-right)))
									(cl-subseq right 1))
								(append left right))))))
		)
			(recur 0 (1- (length array))))))

; Set complement of character set, syntax class, or character category
...
(cl-defstruct (rxt-char-set-negation (:include rxt-char-set)) elt)
...
`rxt-syntax-class', `rxt-char-category', or `rxt-char-set-negation'."
	(cl-etypecase char-set
		...
		(rxt-char-set-negation (rxt-char-set-negation-elt char-set))))
...
(cl-defstruct (rxt-char-set-intersection (:include rxt-char-set)) elts)

; Intersection constructor
(defun rxt-char-set-intersection (&rest charsets)
	(let ((cmpl (make-rxt-char-set-union)) elts)
		(dolist (cset (rxt-int-flatten charsets))
			(cond
				((rxt-char-set-negation-p cset)
					; Fold negated charsets together: ~A & ~B = ~(A|B)
					(setq cmpl
						(rxt-char-set-union cmpl (rxt-char-set-negation-elt cset))))
				((rxt-char-set-union-p cset) (push cset elts))
				(t (rxt-error "Can't take intersection of non-character-set %S" cset))))

		(if (null elts)
			(rxt-negate cmpl)
			(unless (rxt-empty-char-set-p cmpl) (push (rxt-negate cmpl) elts))
			(if (cdr elts)
...
(defun rxt-int-flatten (csets)
	(when (consp csets)
		(let ((cset (car csets)) (tail (rxt-int-flatten (cdr csets))))
...
Returns `nil' if none of the CASES matches."
	...
	`(cond
		,@(mapcar
			(lambda (token_and_action)
				(if (eq (car token_and_action) t)
					(cons t (cdr token_and_action))
					`((looking-at ,(car token_and_action))
						(goto-char (match-end 0))
						,@(cdr token_and_action))))
			cases)))
...
Point should be before the regexp literal before calling this.
Currently only regexps delimited by / ... / are supported.
...
Returns two strings: the regexp and the flags."
	(save-excursion
		...
		; Skip m, qr, s
		(let ((is-subst (rxt-token-case ("s" t) ((rx (or "m" "qr")) nil))))

			(unless (looking-at "/")
				(error "Only Perl regexps delimited by slashes are supported"))
			(let ((beg (match-end 0)) (delim (rx (not (any "\\")) (group "/"))))
...
(defconst rxt-pcre-char-set-alist
	`(
		...
		(?s . (9 10 12 13 32))
		; vertical whitespace
		(?v . (#x000A #x000B #x000C #x000D #x0085 #x2028 #x2029)))
	)
...
(defun rxt-parse-pcre (re) (rxt-parse-re re t))

(defun rxt-parse-elisp (re) (rxt-parse-re re nil))

(defun rxt-parse-re (re pcre-p)
	(let* (
		(rxt-parse-pcre pcre-p)
		...
		(escape (if pcre-p "" "\\"))
		(rxt-choice-regexp (rx-to-string `(seq ,escape "|")))
		(rxt-branch-end-regexp
			(rx-to-string `(or buffer-end (seq ,escape (or "|" ")")))))
		(rxt-brace-begin-regexp (rx-to-string `(seq ,escape "{")))
		...
		(rxt-m-to-?-brace-regexp
			(rx-to-string `(seq (submatch (+ (any "0-9"))) "," ,escape "}")))
		(rxt-m-brace-regexp
			(rx-to-string `(seq (submatch (+ (any "0-9"))) ,escape "}")))
...
(defun rxt-parse-exp ()
	...
	(rxt-with-source-location
		(let (
			(rxt-pcre-extended-mode rxt-pcre-extended-mode)
		...
		(or
			...
			(rxt-token-case
				...
				((rx "\\" (submatch (any ?S ?s)) (submatch nonl))
					(let (
						(negated (string= (match-string 1) "S"))
						(syntax
							(car
								(rassoc
									(string-to-char (match-string 2)) rx--syntax-codes)))
					)
						(if syntax
				...
				((rx "\\" (submatch (any ?C ?c)) (submatch nonl))
					(let (
						(negated (string= (match-string 1) "C"))
						(category
							(car
								(rassoc
									(string-to-char (match-string 2)) rx--categories)))
					)
						(if category
				...
				((rx (seq "\\" (submatch nonl))) (rxt-string (match-string 1)))
				; Normal characters
...
(defun rxt-parse-atom/pcre ()
	...
	(rxt-with-source-location
		(or
			...
			(when-let ((char (rxt-parse-escapes/pcre)))
			...
			(rxt-token-case
				...
				((rx "\\" (submatch (+ (any "0-9"))))
					(let* (
						...
						; as a back reference.
						(if (and (> dec 0) (or (< dec 10) (>= rxt-subgroup-count dec)))
							(progn
								(when rxt-pcre-case-fold
									(display-warning
										'rxt
										"Backreferences with case-folding are handled poorly"))
								(rxt-backref dec))
	...
	(catch 'return
		(let (
			...
			(rxt-token-case
				((rx "?") ; (?
					(rxt-token-case
						((rx ")") (throw 'return (rxt-string ""))) ; Empty group (?)
						(":" (setq shy t)) ; Shy group (?:
						("#" ; Comment   (?#
							(search-forward ")")
							(throw 'return (rxt-string "")))
						((rx
							(or ; Flags     (?isx-isx
							...
							(let (
								...
								(when (cl-find ?x on)  (setq extended-mode t))
								(when (cl-find ?s on)  (setq single-line-mode t))
								(when (cl-find ?i on)  (setq case-fold t))
								(when (cl-find ?x off) (setq extended-mode nil))
								(when (cl-find ?s off) (setq single-line-mode nil))
								(when (cl-find ?i off) (setq case-fold nil))
								(when (string-match-p "[gmo]" token)
									(display-warning
										'rxt
										(format "Unhandled PCRE flags in (?%s" token))))
							(rxt-token-case
								(":" (setq shy t)) ; Shy group with flags (?isx-isx: ...
								(")" ; Set flags (?isx-isx)
									; Set flags for the remainder of the current subexpression
									(setq
										rxt-pcre-extended-mode extended-mode
						...
						(t
							(rxt-error
								"Unrecognized PCRE extended construction `(?%c'"
								(char-after)))))

				; No special (* ...) verbs are recognised
				((rx "*")
					(let ((begin (point)))
						(search-forward ")" nil 0)
		...
		(let ((rx (rxt-parse-exp)))
			(rxt-token-case
				((rx "\\)")
					(cond
						((eq kind 'shy) rx)
						((numberp kind) (rxt-submatch-numbered kind rx))
						(t (rxt-submatch rx))))
				(t (rxt-error "Subexpression missing close paren"))))))
...
(defun rxt-parse-char-class ()
	(when (eobp) (rxt-error "Missing close right bracket in regexp"))
	(rxt-with-source-location
		(let* (
			(negated (rxt-token-case ((rx "^") t)))
			(begin (point))
			(result
				(if negated
					(rxt-negate (rxt-char-set-union))
					(rxt-char-set-union)))
			(transformer (if negated #'rxt-negate #'identity))
			(builder (if negated #'rxt-char-set-intersection #'rxt-choice))
		)
			(catch 'done
				(while t
					(when (eobp) (rxt-error "Missing close right bracket in regexp"))
					(if (and (looking-at (rx "]")) (/= (point) begin))
						(throw 'done result)
						(let ((piece (funcall transformer (rxt-parse-char-class-piece))))
							(setq result (funcall builder result piece))))))
			(forward-char 1) ; Skip over closing "]"
			result)))
...
(defun rxt-parse-char-class-piece ()
	(let ((atom (rxt-parse-char-class-atom)))
		(cl-typecase atom
			(rxt-char-set atom) ; return unchanged
			(integer ; character: check for range
				(if-let ((range-end (rxt-maybe-parse-range-end)))
					(rxt-char-set-union (cons atom range-end))
					(rxt-char-set-union atom)))
			(t (rxt-char-set-union atom))))) ; transform into character set
...
(defun rxt-parse-char-class-atom ()
	(or
		...
		(and rxt-parse-pcre (rxt-parse-char-class-atom/pcre))
		; Char-class syntax
		(rxt-token-case
			; Named classes [:alnum:], ...
			(rxt-named-classes-regexp (intern (match-string 1)))
...
(defun rxt-parse-char-class-atom/pcre ()
	(or
		...
		(rxt-token-case
			...
			((rx "\\" (submatch nonl)) (string-to-char (match-string 1))))))
...
(defun rxt--pcre-char-set (char)
	(let* (
		...
		(if negated (rxt-negate base-char-set) base-char-set)))
...
(defun rxt-rx-symbol (sym)
	(if rxt-verbose-rx-translation
		(or (assoc-default sym rxt-rx-verbose-equivalents) sym)
		sym))
...
(defun rxt-adt->rx (re)
	(let (
		(rx
			(cl-typecase re
				(rxt-primitive (rxt-rx-symbol (rxt-primitive-rx re)))

				(rxt-string
					(if
						(or
							(not (rxt-string-case-fold re))
							(string= "" (rxt-string-chars re)))
						(rxt-string-chars re)
						(cons
							'seq
							(mapcar
								(lambda (char) (list 'any (upcase char) (downcase char)))
								(rxt-string-chars re)))))

				(rxt-seq (cons 'seq (mapcar #'rxt-adt->rx (rxt-seq-elts re))))

				(rxt-choice (cons 'or (mapcar #'rxt-adt->rx (rxt-choice-elts re))))
				...
				(rxt-submatch
					(cons
						...
						(if (rxt-seq-p (rxt-submatch-body re))
							...
							(list (rxt-adt->rx (rxt-submatch-body re))))))
					...
					(cons
						...
						(cons
							...
							(if (rxt-seq-p (rxt-submatch-numbered-p re))
								(mapcar
									#'rxt-adt->rx
									(rxt-seq-elts (rxt-submatch-numbered-body re)))
								(list (rxt-adt->rx (rxt-submatch-numbered-body re)))))))
				...
				(rxt-syntax-class (list 'syntax (rxt-syntax-class-symbol re)))

				(rxt-char-category (list 'category (rxt-char-category-symbol re)))

				(rxt-repeat
					(let (
						(from (rxt-repeat-from re))
						(to (rxt-repeat-to re))
								...
								(rx
									(cond
										...
										((and (zerop from) (equal to 1))
											`(zero-or-one ,body))
										((null to) `(>= ,from ,body))
										((equal from to) `(repeat ,from ,body))
										(t `(** ,from ,to ,body))))
							)
								(if greedy
									(if rxt-explain
										rx ; Readable but not strictly accurate. Fixme?
										(list 'maximal-match rx))
									(list 'minimal-match rx)))
							(cond
								((and (zerop from) (null to))
									`(,(if greedy '* '*?) ,body))
								((and (equal from 1) (null to))
				...
				(rxt-char-set-union
					(let* (
						...
						(if (and (null chars) (null ranges) (= 1 (length classes)))
							(car classes)
							(append '(any) chars ranges classes))))

				(rxt-char-set-negation
					(list 'not (rxt-adt->rx (rxt-char-set-negation-elt re))))

				(t (rxt-error "No RX translation of `%s'" (rxt-to-string re)))))
	)
		...
		(when rxt-explain
			...
			(when (symbolp rx) (setq rx (make-symbol (symbol-name rx))))
...
(defun rxt-adt->pcre (re) (cl-destructuring-bind (s _) (rxt-adt->pcre/lev re) s))

(defun rxt-adt->pcre/lev (re)
	(cl-typecase re
		(rxt-primitive
			(if-let ((s (rxt-primitive-pcre re)))
				(list s 1)
		...
		(rxt-backref (list (format "\\%d" (rxt-backref-n re)) 1))
		(rxt-repeat (rxt-repeat->pcre re))
		((or rxt-char-set-union rxt-char-set-negation) (rxt-char-set->pcre re))
		; FIXME
		; ((rxt-char-set-intersection re) (rxt-char-set-intersection->pcre re))
		(t (rxt-error "No PCRE translation of `%s'" (rxt-to-string re)))))
...
(defun rxt-seq->pcre (re)
	(if-let ((elts (rxt-seq-elts re)))
		(rxt-seq-elts->pcre elts)
		""))

(defun rxt-seq-elts->pcre (elts)
	(cl-destructuring-bind (s lev) (rxt-adt->pcre/lev (car elts))
		(if (cdr elts)
			(cl-destructuring-bind (s1 lev1) (rxt-seq-elts->pcre (cdr elts))
				(list
					(concat
						(rxt-paren-if-necessary s lev)
...
(defun rxt-choice->pcre (re)
	(when-let ((elts (rxt-choice-elts re))) (rxt-choice-elts->pcre elts)))

(defun rxt-choice-elts->pcre (elts)
	(cl-destructuring-bind (s lev) (rxt-adt->pcre/lev (car elts))
		(if (cdr elts)
			(list (concat s "|" (car (rxt-choice-elts->pcre (cdr elts)))) 3)
			(list s lev))))

(defun rxt-submatch->pcre (re)
	(cl-destructuring-bind (s _lev) (rxt-adt->pcre/lev (rxt-submatch-body re))
		(list (concat "(" s ")") 0)))
							...
							(cond
								...
								(t
									(concat
										...
										"}")))
							(cond
								((= from 0) (concat s (if greedy "*" "*?")))
								((= from 1) (concat s (if greedy "+" "+?")))
		...
		(escape
			(lambda (char)
				(let ((s (char-to-string char)))
					(cond
						((string-match rxt-pcre-charset-metachars s) (concat "\\" s))
						((and (not (string= s " ")) (string-match "[^[:graph:]]" s))
							(format "\\x{%x}" char))
						(t s)))))
	)
		(concat
			(mapconcat escape (rxt-char-set-union-chars re))
			(mapconcat
				(lambda (range)
					(concat
						(funcall escape (car range)) "-" (funcall escape (cdr range))))
				(rxt-char-set-union-ranges re))
			(mapconcat
				(lambda (class) (concat "[:" class ":]"))
...
(defun rxt-adt->strings (re)
	(cl-typecase re
		(rxt-primitive (list ""))
		(rxt-string (list (rxt-string-chars re)))
		(rxt-seq (rxt-seq-elts->strings (rxt-seq-elts re)))
		(rxt-choice (rxt-choice-elts->strings (rxt-choice-elts re)))
		(rxt-submatch (rxt-adt->strings (rxt-submatch-body re)))
		(rxt-submatch-numbered (rxt-adt->strings (rxt-submatch-numbered-body re)))
		(rxt-repeat (rxt-repeat->strings re))
		(rxt-char-set-union (rxt-char-set->strings re))
		(t (error "Can't generate productions of %s" (rxt-to-string re)))))

(defun rxt-concat-product (heads tails)
	(mapcan (lambda (hs) (mapcar (lambda (ts) (concat hs ts)) tails)) heads))

(defun rxt-seq-elts->strings (elts)
	(if elts
		(rxt-concat-product
			(rxt-adt->strings (car elts)) ; heads
			(rxt-seq-elts->strings (cdr elts))) ; tails
		'("")))

(defun rxt-choice-elts->strings (elts)
	(when elts
		(append (rxt-adt->strings (car elts)) (rxt-choice-elts->strings (cdr elts)))))

(defun rxt-repeat->strings (re)
	(let ((from (rxt-repeat-from re)) (to (rxt-repeat-to re)))
		(if (not to)
			(error "Can't generate all productions of unbounded repeat \"%s\""
				(rxt-to-string re))
			(rxt-repeat-n-m->strings from to (rxt-adt->strings (rxt-repeat-body re))))))
...
(defun rxt-repeat-n-m->strings (from to strings)
	(cond
		...
		(t ; to > from
			(let* (
				(strs-n (rxt-repeat-n->strings from strings))
...
(defun rxt-repeat-n->strings (n strings)
	...
	(cond
		((zerop n) '(""))
		((= n 1) strings)
		(t (rxt-concat-product (rxt-repeat-n->strings (- n 1) strings) strings))))
...
;	(rxt--read-pcre-mode (if (eq reb-re-syntax 'pcre) 1 0)))
;
