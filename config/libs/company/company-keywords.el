...
; company backend for programming language keywords

(eval-when-compile (require 'make-mode))

(defconst company::keywords::ignore_case nil
    ...
    (f90-mode .
     ...
     ,(let ((keyword_list '(
      ...
      "use" "value" "verify" "volatile" "wait" "where" "while" "with" "write")))
		...
		(nconc (sort (mapcar 'upcase keyword_list) #'string<) keyword_list)))
    (go-mode
...
(defun company-keywords (command &optional arg)
...
(interactive '(interactive))
	(cl-case command
		...
		(prefix
			(and
				(assq major-mode company::keywords::alist)
				(inside_code)
				(company-grab-symbol)))
		(candidates
			(let ((completion-ignore-case company::keywords::ignore_case)
		...
		(kind 'keyword)
		(sorted t)
		(ignore-case company::keywords::ignore_case)))
