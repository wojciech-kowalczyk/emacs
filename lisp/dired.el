...
On systems such as MS-Windows, which use `ls' emulation in Lisp,
...
;; Windows-style drive letters in directory names, like in "d:/foo".
...
If the mark is active, operate on the contents of the region if
`dired-mark-region' is non-nil. Otherwise, operate on the whole buffer.
...
(easy-menu-define dired-mode-operate-menu dired-mode-map
  ...
  '("Operate"
    ...
    ["Change Group..." dired-do-chgrp
     :visible (not (eq system-type 'windows-nt))
...
If the region is active, mark all files
...
this subdir."
  ...
  (cond
   ...
   ((and interactive dired-mark-region mark-active)
    (save-excursion
...
If the region is active, unmark all files in the active region."
  (interactive (list current-prefix-arg t) dired-mode)
...
If the region is active, flag all files in the active region."
  (interactive (list current-prefix-arg t) dired-mode)
  (let ((dired-marker-char dired-del-marker))
    (dired-mark arg interactive)))
...
If the mark is active, operate on the contents
...
If the region is active, mark files
...
If the region is active, flag files
only in the active region if `dired-mark-region' is non-nil."
  (interactive "P" dired-mode)
