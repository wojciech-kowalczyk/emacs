...
If the region is active, this function instead calls `indent-region'.
In this case, any prefix argument is ignored."
  (interactive "P")
  (cond
...
If called interactively with no prefix argument, activate region so
the indentation can be adjusted interactively by typing
\\<indent-rigidly-map>\\[indent-rigidly-left], \\[indent-rigidly-right], \\[indent-rigidly-left-to-tab-stop], or \\[indent-rigidly-right-to-tab-stop].
In addition, \\`TAB' is also bound (and calls `indent-rigidly-right').

Typing any other key exits this mode, and this key is then
acted upon as normally. Exiting also deactivates the mark.
