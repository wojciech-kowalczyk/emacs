	...
	(unless (member attrs normal-top-level-add-subdirs-inode-list)
	  ...
	  (dolist (file contents)
	    (and (string-match "\\`[[:alnum:]]" file)
		 ;; The lower-case variants of RCS and CVS are for Windows.
...
(defun startup--xdg-or-homedot (xdg-dir user-name)
  (let ((emacs-d-dir (concat "~" user-name "/.emacs.d/")))
    (cond
          ...
          (if (eq system-type 'windows-nt)
              (if (file-directory-p (concat "~" user-name))
                  (directory-files (concat "~" user-name) nil
                                   "\\`[._]emacs\\(\\.elc?\\)?\\'"))
            (file-exists-p (concat "~" init-file-user "/.emacs"))))
      emacs-d-dir)
  ...
  (when init-file-user
    (if (string-match "[~/:\n]" init-file-user)
      ...
      (if (file-directory-p (expand-file-name
                             ;; We don't support ~USER on Windows except for the current
