...
If the mark is active, then this function returns a cons-cell of the region.
...
provide a uniform interface to MH-E functions."
  (cond (mark-active (cons (region-beginning) (region-end)))
  ...
  (with-mh-folder-updating (t)
    (save-excursion
      ...
      (if internal-seq-flag
          (progn
            ;; Change the buffer so that if there is an active region
            ;; it will get deactivated as in the case of user sequences.
