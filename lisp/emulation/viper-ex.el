...
(defun ex-write (q-flag)
  ...
  (let ((end (car ex-addresses))
	...
	(unwind-protect
	    (save-excursion
	      (viper-enlarge-region beg end)
	      (setq region (buffer-substring (point) (mark)))
...
(defun ex-print ()
  ...
  (let ((end (car ex-addresses))
    ...
    (save-excursion
	  ...
	  (progn
	    (append-to-buffer viper-ex-print-buf (point) (mark))
		...
		(erase-buffer))))
	(ex-print-display-lines (buffer-substring (point) (mark)))))))
