...
If the mark is active, use the file at the mark as the default for FILE.
(That's the mark set by \\[set-mark-command], not by Dired's \\[dired-mark] command.)
...
If the region is active, mark files only in the active region if
`dired-mark-region' is non-nil."
  (interactive
   (list
...
into the minibuffer."
  (interactive "P" dired-mode)
  (if (and (eq system-type 'windows-nt) (not (file-remote-p default-directory)))
  ...
  (let* ((in-background (string-match "[ \t]*&[ \t]*\\'" command))
	 ...
	 (command (if sequentially
         ...
         (parallel-in-background
          (and in-background (not sequentially)))
	...
	(let* ((char (symbol-value sym))
		...
		(cond ((eq char ?!)
			...
			(t        ; no previous answer - ask now
				(setq prompt
					(concat
						(apply #'format-message prompt args)
						(if-let* (
							help-form
							(help (or help-char (car help-event-list)))
						)
							(format " [Type yn!q or %s] "
								(key-description (vector help)))
							" [Type y, n, q or !] ")))
