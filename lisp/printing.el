...
;;	  option. When this option is on, the `*-buffer*' commands will behave
...
(defun pr-region-active-p ()
  (and pr-auto-region mark-active))
...
(defun pr-dosify-file-name (filename)
  "Replace unix-style directory separator character with Windows one."
...
That is, if Emacs is running on Windows, ensure Windows-style directory
...
Valid values are:
  ...
  :type '(choice :tag "Filename style"
		 (const :tag "Windows Style (\\)" :value windows)
...
COMMAND		Name of the program for printing a text file.  On Windows,
		if the value is an empty string, then Emacs
...
NAME		A string that specifies a text printer name.
		...
		On Windows, it is the name of a printer
...
COMMAND		Name of the program for printing a PostScript file.  On Windows,
...
NAME		A string that specifies a PostScript printer name.
		On Unix-like systems, a string value should be a name
		understood by lpr's -P option (or lp's -d option).
		On Windows, it is the name of a printer device or port.
		Typical non-default settings would be \"LPT1:\"
		to \"LPT3:\" for parallel printers, or \"COM1\" to \"COM4\" or
		\"AUX\" for serial printers, or \"\\\\\\\\hostname\\\\printer\"
		(or \"/D:\\\\\\\\hostname\\\\printer\") for a shared network
		printer.  You can also set it to a name of a file, in which
