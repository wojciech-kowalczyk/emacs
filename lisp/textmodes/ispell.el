...
Interactively, when the mark is active, call `ispell-region' to check the
active region for spelling errors. Non-interactively, this happens if
REGION is non-nil.
...
to limit the check."
  (interactive (when mark-active (list (region-beginning) (region-end))))
...
If region is active, spell-check that region.  Otherwise spell-check the buffer.
