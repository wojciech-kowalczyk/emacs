  ...
  '("Table"
    ...
    ("Recognize"
     ...
     ["in Region" table-recognize-region
      :active (and mark-active (not (eq (mark) (point))))
    ...
    ["Capture Region" table-capture
     :active (and (not buffer-read-only) mark-active (not (eq (mark) (point))) (not (table--probe-cell)))
...
(defun table--cell-insert-char (char &optional overwrite)
  ...
  (let ((delete-selection-p (and delete-selection-mode mark-active (not buffer-read-only)))
	(mark-coordinate (table--transcoord-table-to-cache (table--get-coordinate (mark)))))
    (table-with-cache-buffer
