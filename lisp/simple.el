...
is supplied, or the mark is active."
...
is supplied, or the mark is active."
  ...
  (interactive "^P")
  (or (consp arg) mark-active (push-mark))
  (let ((size (- (point-max) (point-min))))
    (goto-char (if (and arg (not (consp arg)))
		   (- (point-max)
		      (/ (* size (prefix-numeric-value arg)) 10))
...
This affects `delete-forward-char' and `delete-backward-char', though
not `delete-char'.
...
If the mark is active, and N is 1,
...
activating it), unless the mark is already active.
...
When the mark is active, undo changes only within the current region."
  (interactive "*P")
  ...
  (let* ((modified (buffer-modified-p))
    ...
    (unless (and (eq last-command 'undo)
      ...
      (setq undo-in-region
	    (and (or mark-active (and arg (not (numberp arg))))
                 (not inhibit-region)))
      (if undo-in-region
 ...
 `kill-region-dwim'."
  ...
  (interactive (progn
                 (let ((beg (mark))
                       (end (point)))
                   (cond
                    ((and kill-region-dwim (not mark-active))
...
property, in the way that `yank' does."
  ...
  (if (not (eq last-command 'yank))
    ...
    (let ((inhibit-read-only t)
          (before (< (point) (mark))))
      (if before
          (funcall (or yank-undo-function 'delete-region) (point) (mark))
        (funcall (or yank-undo-function 'delete-region) (mark) (point)))
...
If the mark is active, and ARG is 1,
delete the text in the region and deactivate the mark instead.
To disable this, set option `delete-active-region' to nil.
...
(defun mark () "Return this buffer's mark value as integer, or nil if never set."
	(declare (side-effect-free t))
	(marker-position (mark-marker)))

(defun deactivate-mark () "Deactivate the mark, unless it's already inactive."
	(when mark-active
		(setq mark-active nil)
		(run-hooks 'deactivate-mark-hook)
		(set-marker (mark-marker) nil)
		(funcall redisplay-unhighlight-region-function
			(window-parameter nil 'internal-region-overlay)))
	nil)

(defun activate-mark () "Activate the mark, unless it's already active."
	(unless mark-active
		; I added this, because some commands working temporarily on some buffer,
		; setting deactivate-mark locally there, I guess forget that deactivate-mark
		; is only processed by command loop in a buffer that is current after the
		; command ends, so if command ends in a different buffer than it set
		; deactivate-mark in, then deactivate-mark is left there with a non-nil
		; value, and later when some other command switches to that buffer, it will
		; get mark surprisingly deactivated after returning to command loop.
		; So to prevent such unexpected deactivation, reset deactivate-mark here.
...
(defun set-mark (pos) "Set `mark-marker' and `activate-mark'."
...
(defun use-region-noncontiguous-p ()
  ...
  (and mark-active (region-noncontiguous-p)))
...
(defvar redisplay-highlight-region-function
	(lambda (window region_overlay)
		(let (start end)
			(let ((caret (window-point window)) (mark (mark)))
				(if (> caret mark)
					(setq start mark end caret)
					(setq start caret end mark)))
			(if (overlayp region_overlay)
				(unless
					(and
						(eq (overlay-buffer region_overlay) (current-buffer))
						(= (overlay-start region_overlay) start)
						(= (overlay-end region_overlay) end))
					(move-overlay region_overlay start end (current-buffer)))
				(funcall redisplay-unhighlight-region-function region_overlay)
				(setq region_overlay (make-overlay start end))
				(overlay-put region_overlay 'window window)
				(overlay-put region_overlay 'face 'region)
				; Normal priority so that a large region doesn't hide all the
				; overlays within it, but high secondary priority so that if it
				; ends/starts in the middle of a small overlay, that small overlay
				; won't hide the region's boundaries.
				(overlay-put region_overlay 'priority '(nil . 100))))
		region_overlay)
"Function to move the region-highlight overlay.

Called with window and old region overlay, which might be not actually
an overlay.
Create region overlay unless it's already created.
Move overlay to be between point and mark.
Return region overlay.")

(defun sole-buffer-window-p (&optional window buffer)
"Non-nil if WINDOW is the only window displaying BUFFER."
	(unless buffer (setq buffer (current-buffer)))
	(not
		(cl-member-if
			(lambda (window_1) (eq buffer (window-buffer window_1)))
			(cdr (window-list-1 window nil t)))))

(defun redisplay--update-region-highlight (window)
	(let ((region_overlay (window-parameter window 'internal-region-overlay)))
		(if
			(and
				mark-active
				(or
					...
					(if highlight-nonselected-windows
						; If 'window' is not selected and its buffer is displayed
						; in some other window, hide region, because mark and mark-active
						; are buffer-local, not window-local.
						(sole-buffer-window-p window)
						; If minibuffer window is selected, and window was selected
			...
			(let (
				(new_region_overlay
					(funcall redisplay-highlight-region-function window region_overlay))
			)
				(unless (equal new_region_overlay region_overlay)
					(set-window-parameter
						window 'internal-region-overlay new_region_overlay)))
			(funcall redisplay-unhighlight-region-function region_overlay)))
	nil)
...
Activate mark if optional third arg ACTIVATE non-nil."
  (when (mark)
...
A prefix ARG deactivates the mark if it is active, and otherwise avoids
reactivating it."
  (interactive "P")
  (let ((omark (mark)))
    (if (null omark)
...
motion key extends the region. Furthermore, the mark will be deactivated
by any subsequent point motion key that was not shift-translated, or
by any action that normally deactivates the mark.
...
Otherwise, if the region has been activated temporarily, deactivate it."
...
can change the direction by \\[exchange-point-and-mark]."
  ...
  (cond ((and allow-extend
	      (or (and (eq last-command this-command) (mark))
		  mark-active))
