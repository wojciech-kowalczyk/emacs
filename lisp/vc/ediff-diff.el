...
(defcustom ediff-shell
  (cond ((eq system-type 'windows-nt)
	 shell-file-name) ; no standard name
	(t  "sh")) ; UNIX
...
(defcustom ediff-diff-options
  (if (eq system-type 'windows-nt) "--binary" "")
...
The PROGRAM is executed asynchronously unless `system-type' is `windows-nt',
or SYNCH is non-nil.
  ...
  (let ((data (match-data))
    ...
    (unwind-protect
        (with-current-buffer buffer
          ...
          (if (or (eq system-type 'windows-nt) synch)
