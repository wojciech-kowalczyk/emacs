...
current buffer state and calls REPORT-FN when done."
	...
	(let (
		(temp-file (make-temp-file "elisp-flymake-byte-compile"))
		...
		(let* (
			...
			(setq elisp-flymake--byte-compile-process
				(make-process
									...
									(cond
										...
										((zerop (process-exit-status proc))
											(elisp-flymake--byte-compile-done
												report-fn source-buffer output-buffer))
										(t
											(funcall report-fn
												:panic
												:explanation
...
When the mark is active, call `eval-region'.
