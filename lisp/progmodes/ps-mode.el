...
(defconst ps-mode-menu-main
  '("PostScript"
    ...
    "---"
    ["Comment Out Region" ps-mode-comment-out-region (mark)]
    ["Uncomment Region" ps-mode-uncomment-region (mark)]
    "---"
    ["8-bit to Octal Buffer" ps-mode-octal-buffer t]
    ["8-bit to Octal Region" ps-mode-octal-region (mark)]
    "---"
    ["Start PostScript"
     ps-run-start
     t]
    ...
    ["Send Region to Interpreter"
     ps-run-region
     (and (mark) (process-status "ps-run"))]
...
(defun ps-run-start ()
  ...
  (interactive)
  (let ((command (or ps-run-x ps-run-dumb))
	(init-file nil)
