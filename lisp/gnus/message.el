...
citation between (point) and (mark).  And each function should leave
...
(defun message-caesar-region (b e &optional n)
  ...
  (interactive
   (list
    (min (point) (or (mark) (point)))
    (max (point) (or (mark) (point)))
...
However, if `message-yank-prefix' is non-nil, insert that prefix on each line."
  ...
  (if (null message-yank-prefix)
      (indent-rigidly start (or end (mark)) message-indentation-spaces)
    (save-excursion
      (goto-char start)
      (while (< (point) (or end (mark)))
	(cond ((looking-at ">")
