...
(defun proced-mark-all ()
  "Mark all processes. If the region is active, mark the region."
  (interactive nil proced-mode)
  (proced-do-mark-all t))

(defun proced-unmark-all ()
  "Unmark all processes. If the region is active, unmark the region."
  (interactive nil proced-mode)
  (proced-do-mark-all nil))

(defun proced-do-mark-all (mark)
  "Mark all processes using MARK. If the region is active, mark the region."
  (let* ((count 0)
...
If the region is active, omit the processes in region.
If QUIET is non-nil suppress status message.
Returns count of omitted lines."
  ...
  (let ((mark-re (proced-marker-regexp))
        (count 0)
        buffer-read-only)
    (cond (mark-active ;; Omit active region
