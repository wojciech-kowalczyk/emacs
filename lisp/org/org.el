...
If the region is active, promote all headings in the region."
  (interactive)
  (save-excursion
    (if (org-region-active-p)
...
If the region is active, demote all headings in the region."
  (interactive)
  (save-excursion
    (if (org-region-active-p)
        (let ((deactivate-mark nil))
...
ignoring region."
  (interactive
   (cons current-prefix-arg
         (when (and (not current-prefix-arg) mark-active)
           (list (region-beginning) (region-end)))))
  (unless (and beg end)
        ...
        (src-block
         (let ((regionp mark-active))
           (org-babel-do-in-edit-buffer
            ...
            (when (and regionp (not mark-active))
              (push-mark (point-min))
              (goto-char (point-max))
              (setq mark-active t))
...
Interactively, if this command is repeated or if the mark is active,
it marks the next element after the ones already marked."
  (interactive)
  (let (deactivate-mark)
