...
(defun context-menu-undo (menu _click)
  ...
  (when (and (not buffer-read-only)
    ...
    (define-key-after menu [undo]
      `(menu-item ,(if mark-active "Undo in Region" "Undo") undo
...
The highlighting remains as long as the mark remains active.
...
When there is no region, this function does nothing."
  (when mark-active ; Create the secondary selection from the region.
