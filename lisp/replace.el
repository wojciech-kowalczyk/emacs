...
\\<minibuffer-local-map>\\[next-history-element]."
  (delq nil (list (when mark-active
...
Otherwise, operate from point to the end of the buffer's accessible portion.
...
to the end of the buffer).  Interactively, if the mark is active,
operates on the contents of the region;
...
Interactively, when the mark is active, operate
	...
	(setq rend
		...
		(save-excursion
		  ...
		  (unless (or (bolp) (eobp))
		    (forward-line 0))
		  (point-marker)))))
    (if (and interactive mark-active)
...
Interactively, when the mark is active,
...
is not modified."
  (interactive
   (nconc (occur-read-primary-args)
          (and mark-active (list (region-bounds)))))
