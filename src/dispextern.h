...
typedef struct {
	...
	unsigned long color;
	union {
		int size; // For HBAR and BOX.
		struct { int width, height, x_offset; } bar_size; // For BAR.
	} u;
} Cursor;
...
struct glyph {
	...
	// virtual-cursor overlay property.
	// Non-nil to display virtual cursor on this glyph.
	// GLYPH_EQUAL_P tests this with Fequal.
	Lisp_Object virtual_cursor;

	// virtual_cursor from the next (previous if R2L) glyph, if current glyph is
	// covered by virtual cursor, because virtual cursor originally from the
	// next glyph is partially drawn on this glyph. E.g. when drawing bar cursor
	// on the next glyph, like 1 pixel is drawn on this glyph.
	// GLYPH_EQUAL_P tests this with Fequal.
	Lisp_Object virtual_cursor_from_next_glyph;
...
#define GLYPH_EQUAL_P(X, Y) (									\
	...
	/* virtual_cursor prop. */									\
	&& !NILP(Fequal((X)->virtual_cursor, (Y)->virtual_cursor))	\
	&&															\
	!NILP(														\
		Fequal(													\
			(X)->virtual_cursor_from_next_glyph,				\
			(Y)->virtual_cursor_from_next_glyph					\
		)														\
	)															\
)
...
struct glyph_row {
	...
	// If row has at least one glyph with virtual-cursor prop.
	bool_bf has_virtual_cursor : 1;
...
int current_mode_line_height(struct window *);
int current_tab_line_height(struct window *);
int current_header_line_height(struct window *);
...
struct it {
	...
	/* The ID of the default face to use.  DEFAULT_FACE_ID or the face for
	some mode line, depending on what we are displaying.  */
	int base_face_id;
	...
	// virtual-cursor overlay property.
	// Applied to the next glyph not from before/after-strings.
	Lisp_Object virtual_cursor;
};
...
struct redisplay_interface
{
	...
	void (*draw_window_cursor) (struct window *w,
								...
								bool is_active);
	// Optional.
	void (*draw_virtual_cursor) (
		struct window *w, struct glyph_row *row, struct glyph *glyph,
		int hpos, int x, Cursor cursor
	);
...
int estimate_mode_line_height (struct frame *, int);
...
void init_iterator (struct it *, struct window *, ptrdiff_t,
                    ptrdiff_t, struct glyph_row *, int, enum mode_line_type);
...
void move_it_in_display_line(
	struct it *it, ptrdiff_t to_charpos, int to_x, enum move_operation_enum op,
	int vertical_motion
);
void maybe_produce_line_number (struct it *);
bool should_produce_line_number (struct it *);
...
extern void gui_fix_overlapping_area (struct window *, struct glyph_row *,
                                      enum glyph_row_area, int);
extern void draw_filled_box_cursor(struct window *, struct glyph_row *);
extern void get_cursor_geometry(
	struct window *, struct glyph_row const *,
	struct glyph const *, int *, int *, int *, int *
);
...
extern int draw_glyphs(
	struct window *, int, struct glyph_row *, enum glyph_row_area,
	ptrdiff_t, ptrdiff_t, enum draw_glyphs_face, int
);
...
#if defined HAVE_X_WINDOWS || defined USE_CAIRO || defined HAVE_NS || defined HAVE_HAIKU
...
int face_at_string_position (struct window *, Lisp_Object, ptrdiff_t, ptrdiff_t,
                             ptrdiff_t *, int, bool, enum lface_attribute_index);
int merge_faces(struct window *, Lisp_Object, int, int);
int compute_char_face (struct frame *, int, Lisp_Object);
void free_all_realized_faces (Lisp_Object);
extern char unspecified_fg[], unspecified_bg[];

// For highlight_trailing_whitespace.
struct named_merge_point;
bool get_lface_attributes(
	struct window *, struct frame *, Lisp_Object,
	Lisp_Object [LFACE_VECTOR_SIZE], bool, struct named_merge_point *
);
int lookup_face(struct frame *f, Lisp_Object *attr);
void merge_face_vectors(
	struct window *, struct frame *, const Lisp_Object *, Lisp_Object *,
	struct named_merge_point *
);

int merge_face_1(struct window *, struct face *, Lisp_Object);
