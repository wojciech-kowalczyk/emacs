...
The value may be nil, t, a string, a function or a list.

nil means don't display a mode line.

t means display empty mode line - has the same effect as a long string
of spaces.

A string is printed verbatim.

When the value is processed, the window being updated is selected,
and its buffer is current.

A function is called with no args, it can return nil, t, a string which
will be printed verbatim, or a list.
...
Values:
	...
	Regular:
		...
		hbar - horizontal bar cursor with default height of 2
		(hbar . HEIGHT) - horizontal bar cursor with height HEIGHT

		bar -
			vertical bar cursor with default dimensions:
			full height, 3 pixel width, 1 pixel offset to the left -
			equal to (bar 3 0 -1).
		(bar WIDTH HEIGHT X-OFFSET) -
			vertical bar cursor with specified dimensions:
				WIDTH - positive integer, pixel width.
				HEIGHT:
					positive integer -
						pixel height; (row_height - HEIGHT) should be an
						even number, otherwise centering the cursor in
						its row will result in different number of
						spacing pixels below and above the cursor.
					negative integer -
						number of spacing pixels, e.g. -2 means 2 pixels
						below cursor and 2 above cursor;
						if that would make cursor invisible by making
						its height be <= 0, then abort this and just
						make it as high as row.
					0 - the same height as row.
				X-OFFSET -
					pixel offset by which to move cursor.
					0 to display cursor with its left border at the
					first pixel of cursor's main glyph.

			Cursor size is always constrained to row height and
			summed width of main, and, if X-OFFSET is negative,
			previous glyph.

		Anything else - hollow box cursor.

Regular values can be wrapped in a cons where car is the regular value
and cdr is a cursor color to use instead of normal `cursor-color' value.

Other stuff controlling cursor type:
	Window parameter `cursor-type' -
		if non-t, overrides variable cursor-type in that window.
	Variable `cursor-in-non-selected-windows' -
		if non-t, overrides variable and window parameter cursor-type
		in non-selected windows.
	Variable `cursor-type-over-image' -
		if non-t, overrides variable and window parameter cursor-type,
		and variable cursor-in-non-selected-windows when cursor is on
		the image.
	Variable `blink-cursor-alist' -
		alist of (normal_cursor_type . blinked_off_cursor_type).
		blinked_off_cursor_type is like cursor-type value except t
		means hollow cursor.
		normal_cursor_type is set after processing variable and
		window parameter cursor-type, but before final possible
		overriding by cursor-type-over-image.

		The most common value for blink-cursor-alist is just nil,
		meaning all cursor types blinked off are just invisible.  */);
...
to the default frame line height.  A value of nil means add no extra space.  */);
  ...
  DEFVAR_PER_BUFFER ("cursor-in-non-selected-windows",
		     ...
		     doc:
/* `cursor-type' for nonselected windows. See `cursor-type' for details. */);
