...
bool w32_defined_color(
	struct frame *f, const char *color, Emacs_Color *color_def, bool alloc_p,
	bool _makeIndex
) {
...
static BOOL w32_init_class(HINSTANCE hinst) {
	WNDCLASSW  uwc;
...
static void w32_make_gc(struct frame *const f) {
	Emacs_GC gc_values;

	block_input();

	// Create the GC's of this frame.
	gc_values.foreground = f->background_pixel;
	gc_values.background = f->foreground_pixel;
	f->output_data.w32->cursor_gc =
		XCreateGC(NULL, FRAME_W32_WINDOW(f), GCForeground | GCBackground, &gc_values);
...
static UINT_PTR CALLBACK file_dialog_callback(
  ...
  (Lisp_Object prompt, Lisp_Object dir, Lisp_Object default_filename, Lisp_Object mustmatch, Lisp_Object only_dir_p)
{
...

  {
	...
	if (file_details_w->nFilterIndex == 2)
