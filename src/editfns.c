...
//DEFUN(
//	"mark", Fmark, Smark, 0, 0, 0,
//	doc: /* Return value of mark, as an integer or nil. */
//)
//	(void)
//{
//	return Fmark_position(BVAR(current_buffer, mark));
//}
...
//	run_hook(Qdeactivate_mark_hook);
//	unchain_marker(XMARKER(BVAR(current_buffer, mark)));
//	call1(
//		Vredisplay_unhighlight_region_function,
//		window_parameter(XWINDOW(selected_window), Qinternal_region_overlay)
//	);
//}
//
//DEFUN(
//	"set-mark", Fset_mark, Sset_mark, 1, 1,
//	"(goto-char--read-natnum-interactive \"Set mark: \")",
//	doc:
///* Set mark to POSITION - a number or valid marker, or nil meaning to
//deactivate mark.
//
//The return value is POSITION.
//
...
//{
//	if (!NILP(position)) CHECK_FIXNUM_COERCE_MARKER(position);
//
//	bool const is_mark_active = XMARKER(BVAR(current_buffer, mark))->buffer;
//
//	if (NILP(position)) {
//		if (is_mark_active) deactivate_mark();
//	} else {
//		set_marker_restricted(BVAR(current_buffer, mark), position, current_buffer);
//
//		if (!is_mark_active) {
...
{
  Lisp_Object m = Fmarker_position (BVAR (current_buffer, mark));
  if (NILP (m))
