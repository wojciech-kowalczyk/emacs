  ...
  if (pool_changed_p || window_change_flags)
    {
	 ...
	 to the frame width (from CHANGE_FRAME_SIZE).  */
...
static bool update_window(struct window *w, bool force_p) {
...
void gui_update_window_end(
	struct window *w, bool cursor_on_p, bool mouse_face_overwritten_p
) {
	struct frame *f = XFRAME(WINDOW_FRAME(w));
	...
	if (!w->pseudo_window_p) {
		...
		if (cursor_on_p) {
			display_and_set_cursor(
				w, true, w->output_cursor.hpos, w->output_cursor.vpos, w->output_cursor.x,
				w->output_cursor.y
			);
	...
	if (FRAME_RIF(f)->update_window_end_hook)
		FRAME_RIF(f)->update_window_end_hook(w, cursor_on_p, mouse_face_overwritten_p);
}
...
static bool update_text_area(
	struct window *w, struct glyph_row *updated_row, int vpos, bool *partial_p
) {
	...
	else
    {
		...
		bool const is_cursor_at_eol =
			vpos == w->phys_cursor.vpos
			&& (
		...
		if (
			...
			MATRIX_ROW_EXTENDS_FACE_P(desired_row) ? (
				...
				// Do the same if cursor is at the end of this line and call to
				// clear_end_of_line won't erase cursor fully.
				// Use write_glyphs instead of clearing to end of line from
				// w->phys_cursor.x, to handle italic text properly.
				(is_cursor_at_eol && w->phys_cursor.x < desired_row->pixel_width)
		)
			--desired_stop_pos;
		...
		else if (desired_row->pixel_width < current_row->pixel_width)
		{
			...
			int xlim;
			// If cursor is displayed at the end of the line, make sure it's cleared.
			if (is_cursor_at_eol) {
				w->phys_cursor_on_p = false;
				xlim = -1;
			} else
...
static bool update_window_line(struct window *w, int vpos, bool *mouse_face_overwritten_p)
{
	struct glyph_row *current_row = MATRIX_ROW (w->current_matrix, vpos);
	struct glyph_row *desired_row = MATRIX_ROW (w->desired_matrix, vpos);
	struct redisplay_interface *rif = FRAME_RIF (XFRAME (WINDOW_FRAME (w)));
...
static void set_window_cursor_after_update(struct window *w) {
	struct frame *f = XFRAME (w->frame);
	int cx, cy, vpos, hpos;
...
Lisp_Object buffer_posn_from_coords(
	struct window *w, int *x, int *y, struct display_pos *pos, Lisp_Object *object,
		...
		&& EQ (automatic_hscrolling, Qcurrent_line) && IT_CHARPOS (it) < PT)
    {
		...
		move_it_in_display_line (&it2, PT, -1, MOVE_TO_POS, 0);
	...
	{
		// Handle line number margin, because it breaks move_it_in_display_line
		// when window is hscrolled.
		int saved_to_x = to_x;
		// If line numbers should be produced but weren't yet, produce them now.
		// This might be useless in rare cases where maybe_produce_line_number
		// decided to not produce line numbers and that's why it.lnum_pixel_width
		// is 0, not because it wasn't called.
		if (!it.lnum_pixel_width && should_produce_line_number(&it)) {
			maybe_produce_line_number(&it);
			// Cancel unwanted changes from maybe_produce_line_number.
			it.current_x = it.hpos = 0;
		}
		if (it.lnum_pixel_width) {
			// Remove line number margin, for it to not interfere with PRODUCE_GLYPHS.
			it.line_number_produced_p = false;
			saved_to_x = to_x -= it.lnum_pixel_width;
			// If mouse points on line number margin, still get first visible
			// glyph as an object that mouse relates to. X coord will be negative.
			to_x = max(to_x, it.first_visible_x);
			// Shrink window by line number margin width, for continuation lines.
			// I'm not sure this is safe, maybe something checks 'it.w's size?
			// I'm also not sure if this is even needed.
			it.last_visible_x -= it.lnum_pixel_width;
			// Don't modify hpos here to not break move_it_in_display_line.
		}
		move_it_in_display_line (&it, ZV, to_x, MOVE_TO_X, 1);
		if (mouse_prefer_closest_glyph)
	    {
			...
			if (before_dx > after_dx)
				move_it_in_display_line (&it, ZV, next_x, MOVE_TO_X, 1);
	    }

		bidi_unshelve_cache (itdata, 0);
		Fset_buffer (old_current_buffer);

		// Add line number margin columns - they are always visible.
		if (it.lnum_width) it.hpos += it.lnum_width + 2;

		// If mouse points on line number margin, this will be < 0.
		*dx = saved_to_x - it.current_x;
		*dy = *y - it.current_y;
	}
...
void change_frame_size(
	struct frame *f, int new_width, int new_height, bool pretend, bool delay, bool safe
) {
	if (delay || (redisplaying_p && !safe))
    {
		if (CONSP (frame_size_history)
			...
			frame_size_history_extra
				(f, build_string ("change_frame_size, delayed"),
