...
static Lisp_Object read_minibuf(
	Lisp_Object map, Lisp_Object initial, Lisp_Object prompt, bool expflag,
	...
	Vminibuf_scroll_window = minibuf_selected_window = selected_window;
	...
	// This was after run_hook (Qminibuffer_setup_hook), but in at least 1 place
	// I need to set undo list to some saved value, so allow functions in
	// Qminibuffer_setup_hook to modify it.
	...
	// TODO Change this to show minibuffer with inactive face if needed,
	// or don't do that if that's not necessary and just delete this comment.
	// Actually idk why this would be needed - if some command takes long to finish,
	// it should be normally indicated via hourglass or something, not specially here.
...
static void read_minibuf_unwind(void) {
	...
	minibuf_selected_window = Fcar(minibuf_save_list);
	...
	if (NILP (XWINDOW (minibuf_window)->prev_buffers))
    {
		if (!EQ (WINDOW_FRAME (XWINDOW (minibuf_window)), calling_frame))
		{
			...
			// I added this to fix a bug where minibuffer window would stay selected
			// when read-minibuffer-restore-windows is nil and calling window
			// gets deleted while minibuffer is active.
			Lisp_Object win = call2(intern("get-mru-window"), calling_frame, Qt);
			if (!NILP(win))
				Fset_frame_selected_window(calling_frame, win, Qnil);
		}
    }
