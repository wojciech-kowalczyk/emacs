...
UTF-16 before calling any system APIs or CRT library functions.
...
be converted to the ANSI codepage. Otherwise, these variables have no effect
on file-name encoding; this is similar to selection-coding-system.
...
{
	static const char * const tempdirs[] = {"$TMPDIR", "$TEMP", "$TMP"};
	...
	a directory usable for that purpose.  */
	for (i = 0; i < imax; ++i) {
		char const *const tmp = getenv(tempdirs[i] + 1);
	...
	if (i >= imax)
		cmd_error_internal(
			Fcons(
				Qerror,
				Fcons(
					build_string("no usable temporary directories found!!"),
					Qnil
				)
			),
	...
	don't exist.  Fallback on default values where applicable.  */
	{
...

		{
			...
			// of Windows 95 and NT4 that have not been updated to include MSIE 5.
			ShGetFolderPath_fn get_folder_path =
				(ShGetFolderPath_fn)
				get_proc_addr(GetModuleHandle("shell32.dll"), "SHGetFolderPathA");

			if (
				get_folder_path != NULL
				&& get_folder_path(NULL, CSIDL_APPDATA, NULL, 0, default_home) == S_OK
			)
		...
		// Treat emacs_dir specially: set it unconditionally based on our location.
		{
			char *p;
			char modname[MAX_PATH];

			if (!GetModuleFileNameA(NULL, modname, MAX_PATH)) emacs_abort();
			if ((p = _mbsrchr (modname, '\\')) == NULL) emacs_abort();
			*p = 0;
			...
			if (
				...
				*p = 0;
				...
				if (within_build_tree) {
					_snprintf(buf, sizeof buf - 1, "SHELL=%s/nt/cmdproxy.exe", modname);
					_putenv(strdup(buf));
				}
			}
		}
		...
		for (i = 0; i < N_ENV_VARS; ++i)
			if (!getenv(env_vars[i].name)) {
				...
				if (
					(lpval = w32_get_resource(REG_ROOT, env_vars[i].name, &dwType))
					== NULL
...
char *emacs_root_dir (void) {
	static char root_dir[MAX_UTF8_PATH];
	const char *p;

	p = getenv("emacs_dir");
	if (p == NULL) emacs_abort();
...
sys_readdir (DIR *dirp)
{
	...
	else if (!FindNextFileW (dir_find_handle, &dir_find_data_w)) {
		errno = 0;
		return NULL;
	}
...
static int sys_access(char const *const fname, int const mode) {
	char fname_copy[MAX_PATH];
	strcpy(fname_copy, fname);
	unixtodos_filename(fname_copy);

	DWORD attributes = GetFileAttributesA(fname_copy);
	...
	if ((mode & D_OK) && !(attributes & FILE_ATTRIBUTE_DIRECTORY)) {
		errno = EACCES;
		return -1;
	}
	return 0;
...
symlink (char const *filename, char const *linkname)
{
	...
	DWORD flags =
		w32_major_version > 10 || (w32_major_version == 10 && w32_build_number >= 14972)
...
readlink (const char *name, char *buf, size_t buf_size)
{
	...
	errno = 0;
	if (!(volume_info.flags & FILE_SUPPORTS_REPARSE_POINTS) || !is_symlink(path))
    {
		if (!errno)
...
bool symlinks_supported(const char *file)
{
	return
		get_volume_info(file, NULL) && volume_info.flags & FILE_SUPPORTS_REPARSE_POINTS;
}
...
/* Return a handle to GDI32.DLL.  */
HANDLE maybe_load_unicows_dll(void) {
