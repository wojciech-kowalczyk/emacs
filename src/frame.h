...
struct frame
{
	...
	On ttys and on Windows, to avoid wasting effort updating
	...
	Cursor cursor; // Desired cursor. Has to be specified (I think).
	// For one weird case in display_and_set_cursor.
	// Set together with 'cursor' above.
	//
...
{
	return f->right_divider_width;
}

INLINE int
FRAME_BOTTOM_DIVIDER_WIDTH (struct frame *f)
{
	return f->bottom_divider_width;
}
