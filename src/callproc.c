...
/* The next variable is used while record-unwind-protect is in place
   during call-process for a subprocess for which record_deleted_pid has
   not yet been called. Doing this via static
...
void record_kill_process(struct Lisp_Process *p) {
	sigset_t oldset;
	block_child_signal (&oldset);

	if (p->alive)
    {
		record_deleted_pid (p->pid, Qnil);
		p->alive = 0;
		kill (- p->pid, SIGKILL);
    }
...
#ifdef WINDOWSNT
	       /* Windows sometimes return EACCES for bad file names  */
	       && errno != EACCES
#endif
	       )
