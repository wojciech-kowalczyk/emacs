...
static Lisp_Object command_loop_1(void) {
	...
	while (true)
    {
		...
		if (!NILP (BVAR (current_buffer, mark_active)) && !NILP(Vrun_hooks))
//		if (XMARKER(BVAR(current_buffer, mark))->buffer && !NILP(Vrun_hooks))
		{
			if (!NILP(Vdeactivate_mark))
				/* If `select-active-regions' is non-nil, this call
				also sets the PRIMARY selection.  */
...
//				deactivate_mark();
			else
			{
				...
				if ((!NILP (Fwindow_system (Qnil))
					...
					&& NILP (Fmemq (Vthis_command, Vselection_inhibit_update_commands)))
				{
					Lisp_Object txt
...
static Lisp_Object make_lispy_position(
	struct frame *f, Lisp_Object x, Lisp_Object y, Time t
) {
	...
	if (WINDOWP (window_or_frame))
    {
		...
		if (!textpos)
		{
			...
			in text-area, right-margin/fringe, vertical border and right-side
			vertical scroll bar, zero otherwise.  */
			int x2 =
				part == ON_TEXT
				? xret
				:
					part == ON_RIGHT_FRINGE
		...
		extra_info =
			Fcons(
				string_info,
				Fcons(
					textpos < 0 ? Qnil : make_fixnum (textpos),
					Fcons(Fcons(make_fixnum (col), make_fixnum (row)), extra_info)
				)
			);
    }
	else if (f)
	...
	return
		Fcons(
			...
			Fcons(
				posn,
				Fcons(
					Fcons(make_fixnum(xret), make_fixnum (yret)),
					Fcons(INT_TO_INTEGER(t), extra_info)
				)
			)
		);
}
...
make_lispy_event (struct input_event *event)
{
  ...
  switch (event->kind)
    ...
    /* On Windows, NO_EVENT is used to delete extraneous
...
function is called to remap that sequence.  */);
  ...
  DEFSYM (Qdeactivate_mark_hook, "deactivate-mark-hook");
