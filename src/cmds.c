...
`post-self-insert-hook'.  */
	...
	CHECK_FIXNUM(n);
	if (XFIXNUM(n) <= 0) return Qnil;

	Fbarf_if_buffer_read_only(Qnil);

	// Backward compatibility.
	if (NILP(c)) c = last_command_event;
...
//	if (XMARKER(BVAR(current_buffer, mark)->buffer)) {
//		Fdelete_region(Fregion_beginning(), Fregion_end());
//		deactivate_mark();
//	}
	...
	{
		int c1 = XFIXNUM(c);
		EMACS_INT n1 = XFIXNAT(n);

		bool hairy = false;
		Lisp_Object tem;
		register enum syntaxcode synt;
		Lisp_Object overwrite = BVAR(current_buffer, overwrite_mode);
		// Length of multi-byte form of 'c1'.
		int len;
		// Working buffer and pointer for multi-byte form of 'c1'.
		unsigned char str[MAX_MULTIBYTE_LENGTH];
		ptrdiff_t chars_to_delete = 0;
		ptrdiff_t spaces_to_insert = 0;

		// At first, get multi-byte form of 'c1' in 'str'.
		if (!NILP(BVAR(current_buffer, enable_multibyte_characters))) {
			len = CHAR_STRING(c1, str);
			if (len == 1)
				// If 'c1' has modifier bits, this makes 'c1' an appropriate
				// one-byte char.
				c1 = *str;
		} else {
			str[0] = SINGLE_BYTE_CHAR_P(c1) ? c1 : CHAR_TO_BYTE8(c1);
			len = 1;
		}
		if (!NILP(overwrite) && PT < ZV) {
			// In overwrite-mode, we substitute a character at point ('c2',
			// hereafter) by 'c1'. For that, we delete 'c2' in advance. But,
			// just substituting 'c2' by 'c1' may move a remaining text in the
			// line to the right or to the left, which is not preferable.
			// So we insert more spaces or delete more characters in the
			// following cases: if 'c1' is narrower than 'c2', after deleting
			// 'c2', we fill columns with spaces, if 'c1' is wider than 'c2',
			// we delete 'c2' and several characters following 'c2'.
			...
			// Overwriting in binary-mode always replaces 'c2' by 'c1'.
			...
			if (EQ(overwrite, Qoverwrite_mode_binary))
				chars_to_delete = min(n1, PTRDIFF_MAX);
			else if (
				c1 != '\n' && c2 != '\n'
				&& (cwidth = XFIXNAT(Fchar_width(make_fixnum(c1)))) != 0
			) {
				ptrdiff_t pos = PT;
				ptrdiff_t pos_byte = PT_BYTE;
				ptrdiff_t curcol = current_column();

				if (n1 <= (min(MOST_POSITIVE_FIXNUM, PTRDIFF_MAX) - curcol) / cwidth) {
					// Column the cursor should be placed at after this insertion.
					// The value should be calculated only when necessary.
					ptrdiff_t target_clm = curcol + n1 * cwidth;
		...
		if (
			!NILP(BVAR(current_buffer, abbrev_mode))
			&& synt != Sword
			...
			if (
				SYMBOLP(sym) && !NILP(sym) && !NILP(XSYMBOL(sym)->u.s.function)
				&& SYMBOLP(XSYMBOL(sym)->u.s.function)
			) {
				Lisp_Object prop;
				prop = Fget(XSYMBOL(sym)->u.s.function, intern("no-self-insert"));
				if (!NILP(prop)) goto exit;
			}

			if (MODIFF != modiff) hairy = true;
		}
		...
		if (chars_to_delete) {
			int mc =
				...
				? UNIBYTE_TO_CHAR(c1)
				: c1;
			Lisp_Object string = Fmake_string(make_fixnum(n1), make_fixnum(mc), Qnil);

			if (spaces_to_insert) {
				tem = Fmake_string(make_fixnum(spaces_to_insert), make_fixnum(' '), Qnil);
				string = concat2(string, tem);
			}

			ptrdiff_t to;
			if (INT_ADD_WRAPV(PT, chars_to_delete, &to)) to = PTRDIFF_MAX;
			replace_range(PT, to, string, 1, 1, 1, 0, false);
			Fforward_char(make_fixnum (n1));
		} else if (n1 > 1) {
			USE_SAFE_ALLOCA;
			char *strn, *p;
			SAFE_NALLOCA(strn, len, n1);
			for (p = strn; n1 > 0; n1--, p += len) memcpy(p, str, len);
		...
		if (
			(
				CHAR_TABLE_P(Vauto_fill_chars)
				? !NILP(CHAR_TABLE_REF(Vauto_fill_chars, c1))
				: (c1 == ' ' || c1 == '\n')
			)
			...
			if (c1 == '\n' && PT < ZV) SET_PT_BOTH(PT + 1, PT_BYTE + 1);

			hairy = !NILP(auto_fill_result);
		}

		// Run hooks for electric keys.
		run_hook(Qpost_self_insert_hook);

		if (hairy) Fset(Qundo_auto__this_command_amalgamating, Qnil);
	}
	exit:
