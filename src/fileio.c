...
DEFUN(
	"file-name-directory", Ffile_name_directory, Sfile_name_directory,
	1, 1, 0,
...
Given a Unix syntax file name, returns a string ending in slash.  */
	...
	if (!NILP(handler)) {
		Lisp_Object handled_name = call2(handler, Qfile_name_directory, filename);
...
Lisp_Object file_name_directory(Lisp_Object filename) {
	char *beg = SSDATA(filename);
	char const *p = beg + SBYTES(filename);
	while (
		p != beg && !IS_DIRECTORY_SEP(p[-1])
#ifdef WINDOWSNT
	...
	if (p[-1] == ':') {
		...
		if (getdefdir(c_toupper(*beg) - 'A' + 1, r)) {
			...
			tem_fn = make_specified_string(beg, -1, p - beg, STRING_MULTIBYTE(filename));
		} else
			tem_fn =
				make_specified_string(
					beg - 2, -1, p - beg + 2, STRING_MULTIBYTE(filename)
				);
	} else if (STRING_MULTIBYTE(filename)) {
		tem_fn = make_specified_string(beg, -1, p - beg, 1);
		dostounix_filename(SSDATA(tem_fn));
		if (!NILP(Vw32_downcase_file_names)) tem_fn = Fdowncase(tem_fn);
	} else {
		dostounix_filename(beg);
...
DEFUN(
	"file-name-nondirectory", Ffile_name_nondirectory, Sfile_name_nondirectory,
	1, 1, 0,
...
or the entire name if it contains no slash.  */
	...
	if (!NILP(handler)) {
		Lisp_Object handled_name = call2(handler, Qfile_name_nondirectory, filename);
		if (STRINGP(handled_name)) return handled_name;
...
static ptrdiff_t file_name_as_directory(
	char *dst, char const *src, ptrdiff_t srclen, bool multibyte
) {
	if (srclen == 0) {
		dst[0] = '.';
		dst[1] = '/';
  ...
  (Lisp_Object name, Lisp_Object default_directory)
{
  ...
  Lisp_Object root;
#ifdef WINDOWSNT
      /* "/" is not considered a root directory on WINDOWSNT, so using it
  ...
  /* At this point, nm might or might not be an absolute file name.  We
     ...
     We set newdir to be the appropriate prefix if one is needed:
       ...
       - the specified drive's working dir (WINDOWSNT only) if nm does not
         start with /
...
#ifdef WINDOWSNT
  /* On Windows, nm is absolute if a drive name was specified;
  ...
  if (
      ...
      !(IS_DIRECTORY_SEP (nm[0]) && IS_DIRECTORY_SEP (nm[1])
	   && !IS_DIRECTORY_SEP (nm[2]))
#else
      /* /... alone is not absolute on Windows.  */
      !IS_DIRECTORY_SEP (nm[0])
#endif
      && !newdir
  )
		     ...
		     || p[2] == 0))
	  {
	    ...
	    p += 2;
		 ...
		 /* `/../' is the "superroot" on certain file systems.
		    Turned off on WINDOWSNT systems because they have no
   ...
   Lisp_Object preserve_permissions)
{
  ...
  if (!NILP (preserve_permissions))
    {
#if HAVE_LIBSELINUX
      if (is_selinux_enabled() && emacs_fd_to_int (ifd) != -1)
	{
  ...
  if (emacs_fd_to_int (ifd) != -1
      ...
      if (emacs_fd_to_int (ifd) != -1)
	{
	 ...
	 input is empty and so could be a /proc file.
	  read+write will either succeed, or report an error more precisely
	  than copy_file_range would.  */
...
file_name_case_insensitive_err (Lisp_Object file)
{
  /* Filesystems are case-sensitive on all supported systems except
     MS-Windows, Cygwin, and macOS.  They are always
...
file_directory_p (Lisp_Object file)
{
  ...
  if (!retval && errno == EACCES)
    errno = ENOTDIR;	/* like the non-WINDOWSNT branch below does */
  return retval;
#else
# ifdef O_PATH
...
On MS-Windows, binary mode is needed to read or write
...
On MS-Windows, the value of this variable is largely ignored.
Emacs on Windows behaves as if file names were encoded in `utf-8'.  */);
