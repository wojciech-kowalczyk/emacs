...
Using an Emacs configured with --with-x-toolkit=lucid does not have this problem.\n",
  ...
  /* Do not make gmalloc thread-safe when creating bootstrap-emacs, as
     that causes an infinite recursive loop with FreeBSD.  See Bug#14569.  */
  ...
  if (!initialized)
    {
	 ...
	 function creates Vterminal_frame.  */
      syms_of_xfaces ();
...
# if !defined SYSTEM_MALLOC && !defined WINDOWSNT
...
both `lib-src' and `etc' directories are found
