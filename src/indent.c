...
DEFUN ("vertical-motion", Fvertical_motion, Svertical_motion, 1, 4, 0,
...
If CLOSER_P is non-nil and destination column is specified, then point
will be left on the position closest to the destination column,
not like originally always at or before it.  */)
(Lisp_Object lines, Lisp_Object window, Lisp_Object cur_col, Lisp_Object closer_p) {
	struct it it;
	struct text_pos pt;
	struct window *w;
	...
	else
    {
		...
		if (!NILP (lcols))
			to_x = window_column_x(w, window, XFLOATINT(lcols), lcols) + lnum_pixel_width;
		if (nlines <= 0)
		{
			it.vpos = vpos_init;
			it.current_y = 0;
		...
		if (!NILP (lcols)) {
			to_x += first_x; // Add hscroll.
			if (lnum_pixel_width) {
				// Remove line number margin, for it to not
				// interfere with PRODUCE_GLYPHS.
				it.line_number_produced_p = false;
				to_x -= lnum_pixel_width;
				...
				it.last_visible_x -= lnum_pixel_width;
			}

			int const arg = NILP(closer_p) ? 1 : 1 | 2;
			move_it_in_display_line(&it, ZV, to_x, MOVE_TO_X, arg);
