...
struct window {
	...
	// Mark marker. Like 'pointm' but for mark.
//    Lisp_Object mark;
    ...
    /* Last cursor drawn on the window.  */
    Cursor phys_cursor_type;

    int phys_cursor_width, phys_cursor_height, phys_cursor_ascent;

	// x position of the mostleft affected glyph, not x position of displayed
	// like phys_cursor.x.
	// To me it seemed like some functions checking for intersections with
	// cursor rectangle incorrectly took phys_cursor.x.
	int phys_cursor_glyph_x;
	// There should be corresponding variable for y position.
	// Now phys_cursor.y is incorrectly set to the top of the row.

	// Added last minute because some things related to display_and_set_cursor
	// need that to remember the base glyph x coord that cursor was requested
	// to be displayed on.
	int phys_cursor_main_glyph_x;

	// True if cursor covers 2 glyphs - (phys_cursor.hpos - 1) and phys_cursor.hpos;
	// for R2L - phys_cursor.hpos and (phys_cursor.hpos + 1).
	bool phys_cursor_between_p;
...
#if defined USE_TOOLKIT_SCROLL_BARS || defined HAVE_NTGUI
