...
w32_set_cursor_gc (struct glyph_string *s)
{
	...
	else
	{
		...
		egc.background = s->w->phys_cursor_type.color;
		...
		if (egc.foreground == egc.background)
			egc.foreground = s->f->background_pixel;
		if (egc.foreground == egc.background)
			egc.foreground = s->face->foreground;
...
w32_draw_glyph_string_box (struct glyph_string *s)
{
	...
	// [Wrong mouse face box] A big problem: old version below was wrong,
	// because mouse highlighting drawing something like this:
	// #("asd" 0 1 (face (:box t)) 1 2 (face (:box t) mouse-face (:foreground "red"))
	// 2 3 (face (:box t)))
	// meaning mouse face w/o :box specified, in a string with box,
	// drawn box walls for mouse face.
	// TODO So now functions setting up HLINFO and calling show_mouse_face with it,
	// are expected to temporarily set left_box_line_p and right_box_line_p
	// flags according to the 'face and 'mouse-face values.
	// I implemented it only in mode mode handling branch of note_mouse_highlight,
	// as that was the thing that I cared for at the moment, but the normal
	// in buffer handling branch is still wrong there.
//	left_p = (s->first_glyph->left_box_line_p
	...
	if (s->img->pixmap)
	{
		...
		if (s->img->mask)
		{
			...
			else
			{
				...
				if ((pmode = SetStretchBltMode (s->hdc, HALFTONE)) != 0)
...
static void w32_draw_glyph_string(struct glyph_string *s) {
...
static int w32_note_mouse_movement(struct frame *frame, MSG *msg) {
	...
	while (get_next_msg (&msg, FALSE))
	{
		...
		switch (msg.msg.message)
		{
			...
			else
			{
				...
				// Also set mouse pointer to the default one.
				// clear_mouse_face sets hlinfo->mouse_face_window to nil.
				Lisp_Object window = hlinfo->mouse_face_window;
				clear_mouse_face(hlinfo);
				if (!NILP(window)) {
					struct frame *f = XFRAME(WINDOW_FRAME(XWINDOW(window)));
					FRAME_RIF(f)->define_frame_cursor(
						f, FRAME_OUTPUT_DATA(f)->nontext_cursor
					);
				}
			}
...
static void w32_draw_window_cursor(
	struct window *const w, struct glyph_row *const row, struct glyph *const glyph,
	bool const is_active
) {
	struct frame *const f = XFRAME(w->frame);
	...
	if (is_active) {
		...
		w32_system_caret_x = WINDOW_TEXT_TO_FRAME_PIXEL_X(w, w->phys_cursor.x);
		w32_system_caret_y = WINDOW_TO_FRAME_PIXEL_Y(w, row->y);
		...
		// If the size of the active cursor changed, destroy the old system caret.
		if (w32_system_caret_hwnd && w32_system_caret_height != w->phys_cursor_height)
	...
	and mini-buffer.  */
	if (w->phys_cursor_type.kind == NO_CURSOR || !glyph) {
		w->phys_cursor_width = 0;
		return;
	}
	if (w->phys_cursor_type.kind == FILLED_BOX_CURSOR) {
		f->output_data.w32->cursor_gc->background = w->phys_cursor_type.color;
		draw_filled_box_cursor(w, row);
		return;
	}
	...
	switch (w->phys_cursor_type.kind) {
		case HOLLOW_BOX_CURSOR: {
			/* Compute frame-relative coordinates for phys cursor.  */
			int x = w->phys_cursor.x, y, height, width;
			get_cursor_geometry(w, row, glyph, &x, &y, &height, &width);
			...
			if (row->reversed_p && glyph->pixel_width > width)
				x += glyph->pixel_width - width;

			RECT rect = {x, y, x + width, y + height};
			HBRUSH hb = CreateSolidBrush(w->phys_cursor_type.color);
		...
		case HBAR_CURSOR: {
			int width;
			{
				int _x = w->phys_cursor.x, _y, _height;
				get_cursor_geometry(w, row, glyph, &_x, &_y, &_height, &width);
			}
			w->phys_cursor_width = width;
			...
			int const height = min(row->height, max(0, w->phys_cursor_type.u.size));
			w32_fill_area(
				f, hdc, w->phys_cursor_type.color, x,
				WINDOW_TO_FRAME_PIXEL_Y(w, row->y + row->height - height), width, height
			);
		...
		default: // BAR_CURSOR
			...
			// Handle height.
			int height = w->phys_cursor_type.u.bar_size.height;
			if (height == 0)
				height = row->height;
			else if (height > 0)
				height = min(height, row->height);
			else {
				height = row->height + height * 2;
				if (height <= 0)
					height = row->height;
			}
			// Center vertically.
			// Could add support for a specific y-offset.
			int const y = row->y - (height - row->height) / 2;

			int width = w->phys_cursor_type.u.bar_size.width;
			int const x_offset = w->phys_cursor_type.u.bar_size.x_offset;
			int const x = WINDOW_TEXT_TO_FRAME_PIXEL_X(w, w->phys_cursor.x);
			int const x_max = x + glyph->pixel_width;
			int x_real = x + x_offset;
			int const saved_phys_cursor_glyph_x = w->phys_cursor_glyph_x;

			if (x_offset < 0) {
				// Move x_real to its correct position and clip width from the left.
				// If cursor is on the first glyph in its row.
				if (
					glyph
					==
					row->glyphs[TEXT_AREA]
					- (row->reversed_p ? row->used[TEXT_AREA] - 1 : 0)
				) {
					width += x_offset;
					x_real -= x_offset;
				} else {
					w->phys_cursor_between_p = true;

					if (!row->reversed_p)
						w->phys_cursor_glyph_x -= (glyph - 1)->pixel_width;

					int const x_min =
						x - (glyph + (row->reversed_p ? 1 : -1))->pixel_width;
					if (x_real < x_min) {
						width -= x_min - x_real;
						x_real = x_min;
					}
			...
			// Clip width from the right.
			width = min(width, x_max - x_real);

			// Abort if cursor dimensions are wrong.
			if (x_real >= x_max || width <= 0) {
				// Not sure if that's actually used, but maybe some rectangle
				// intersection stuff benefits from this because it never
				// actually intersects, so cursor glyph doesn't need to be redrawn?
				w->phys_cursor_width = 0;

				w->phys_cursor_between_p = false;
				w->phys_cursor_glyph_x = saved_phys_cursor_glyph_x;

				break;
			}

			// If R2L, mirror 'x_real' to the right side of glyph.
			if (row->reversed_p) {
				// Left part's width as for L2R.
				int const left_width = x - x_real;
				x_real += glyph->pixel_width - (left_width - (width - left_width));
			}

			w->phys_cursor_width = width;
			w->phys_cursor.x += x_real - x;

			w32_fill_area(
				f, hdc, w->phys_cursor_type.color, x_real,
				WINDOW_TO_FRAME_PIXEL_Y(w, y), width, height
			);
	}
...
// Draw virtual cursor.
// The same as w32_draw_window_cursor but doesn't use w->phys_cursor... stuff.
// Obviously these two should share more code.
static void w32_draw_virtual_cursor(
	struct window *const w, struct glyph_row *const row, struct glyph *const glyph,
	int const hpos, int x, Cursor const cursor
) {
	struct frame *const f = XFRAME(w->frame);
	if (cursor.kind == FILLED_BOX_CURSOR) {
		unsigned long const saved_color = f->output_data.w32->cursor_gc->background;
		f->output_data.w32->cursor_gc->background = cursor.color;
		// Based on draw_filled_box_cursor.
		draw_glyphs(w, x, row, TEXT_AREA, hpos, hpos + 1, DRAW_CURSOR, 0);
		f->output_data.w32->cursor_gc->background = saved_color;
		return;
	}
	...
	switch (cursor.kind) {
		case HOLLOW_BOX_CURSOR: {
			...
			RECT rect = {x, y, x + width, y + height};
			HBRUSH hb = CreateSolidBrush(cursor.color);
			FrameRect(hdc, &rect, hb);
			DeleteObject(hb);
		...
		case HBAR_CURSOR: {
			...
			int const height = min(row->height, max(0, cursor.u.size));
			w32_fill_area(
				f, hdc, cursor.color, x,
				WINDOW_TO_FRAME_PIXEL_Y(w, row->y + row->height - height),
				width, height
			);
		...
		default: // BAR_CURSOR
			// Handle height.
			int height = cursor.u.bar_size.height;
			if (height == 0)
				height = row->height;
			else if (height > 0)
				height = min(height, row->height);
			...
			x = WINDOW_TEXT_TO_FRAME_PIXEL_X(w, x);
			int width = cursor.u.bar_size.width;
			int const x_offset = cursor.u.bar_size.x_offset;
			int const x_max = x + glyph->pixel_width;
			int x_real = x + x_offset;
			if (x_offset < 0) {
				// Move x_real to its correct position and clip width
				// from the left.
				// If cursor is on the first glyph in its row.
				if (
					glyph
			...
			w32_fill_area(
				f, hdc, cursor.color, x_real, WINDOW_TO_FRAME_PIXEL_Y(w, y), width, height
			);
	}
...
frame_set_mouse_pixel_position (struct frame *f, int pix_x, int pix_y)
{
	...
	if (w32_major_version + w32_minor_version >= 6)
