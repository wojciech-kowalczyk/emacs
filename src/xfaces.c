...
static struct face *realize_non_ascii_face (struct frame *, Lisp_Object, struct face *);
...
static unsigned long load_color2(
	struct frame *f, struct face *face, Lisp_Object name,
...
bool get_lface_attributes(
	struct window *w, struct frame *f, Lisp_Object face_name,
	...
	if (CONSP(face_remapping)) {
		...
		if (
			...
			return
				merge_face_ref(
					w, f, XCDR(face_remapping), attrs, signal_p, named_merge_points, 0
				);
		}
    }
...
void merge_face_vectors(
	struct window *w, struct frame *f, const Lisp_Object *from, Lisp_Object *to,
	struct named_merge_point *named_merge_points
) {
	int i;
...
int face_at_string_position(
	struct window *w, Lisp_Object string, ptrdiff_t pos, ptrdiff_t bufpos,
	ptrdiff_t *endptr, int base_face_id, bool mouse_p,
	enum lface_attribute_index attr_filter
) {
...
int merge_face_1(
	struct window *const window, struct face *const base_face, Lisp_Object face
) {
	Lisp_Object attrs[LFACE_VECTOR_SIZE];
	...
	struct frame *const frame = XFRAME(window->frame);
	/* Merge in attributes specified via text properties.  */
	merge_face_ref(window, frame, face, attrs, true, NULL, 0);

	/* Look up a realized face with the given face attributes,
	or realize a new one for ASCII characters.  */
	return lookup_face(frame, attrs);
}
