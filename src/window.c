...
wset_combination (struct window *w, bool horflag, Lisp_Object val)
{
  ...
  eassert (!BUFFERP (w->contents) && NILP (w->start) && NILP (w->pointm)); // && NILP(w->mark));
  w->contents = val;
...
wset_buffer (struct window *w, Lisp_Object val)
{
  ...
  if (BUFFERP (val))
    ...
    eassert (MARKERP (w->start) && MARKERP (w->pointm)); // && MARKERP(w->mark));
  w->contents = val;
  adjust_window_count (w, 1);
}
...
static Lisp_Object select_window(
	Lisp_Object window, Lisp_Object norecord, bool inhibit_point_swap
) {
	struct window *w;
	struct frame *sf;
	Lisp_Object frame;
    ...
    { /* Mark the window for redisplay since the selected-window has
	  a different mode-line.  */
		...
		wset_redisplay(w);
		// If minibuffer window is going to be selected or deselected,
		// update mode-line of minibuf_selected_window.
		if (
			minibuf_level > 0
			&& (MINI_WINDOW_P(XWINDOW(selected_window)) || MINI_WINDOW_P(w))
			&& WINDOW_LIVE_P(minibuf_selected_window)
		)
			wset_redisplay(XWINDOW(minibuf_selected_window));
    }
	else
...
/* Select window with a minimum of fuss, i.e. don't record the change anywhere
   ...
   already selected.  */
static void select_window_1(Lisp_Object window, bool inhibit_point_swap) {
	...
	if (!inhibit_point_swap) {
		struct window *const w = XWINDOW(selected_window);
		if (BUFFERP(w->contents)) {
			set_marker_both(
				w->pointm, w->contents,
				BUF_PT(XBUFFER(w->contents)), BUF_PT_BYTE(XBUFFER(w->contents))
			);
			// Do the same with mark.
//			Fset_marker(w->mark, BVAR(XBUFFER(w->contents), mark), w->contents);
		}
    }
	...
	struct window *const w = XWINDOW(window);
	set_point_from_marker(w->pointm);
	// Do the same with mark.
//	Fset_marker(BVAR(current_buffer, mark), w->mark, current_buffer);
}
...
//DEFUN(
//	"window-mark", Fwindow_mark, Swindow_mark, 0, 1, 0,
//	doc: /* `window-point' for `mark'. */
//)
//	(Lisp_Object window)
//{
//	struct window *const w = decode_live_window(window);
//	return
//		Fmarker_position(
//			w == XWINDOW(selected_window)
//			? BVAR(XBUFFER(w->contents), mark)
//			: w->mark
//		);
//}
...
DEFUN(
	"set-window-point", Fset_window_point, Sset_window_point, 2, 2, 0,
	doc:
/* Make point value in WINDOW be at position POS in WINDOW's buffer.
WINDOW must be a live window and defaults to the selected one.
Return POS. POS must be an integer or valid marker. */
)
	(Lisp_Object window, Lisp_Object pos)
{
	struct window *const w = decode_live_window(window);

	// Fgoto_char checks type.
	if (w == XWINDOW(selected_window)) {
		if (XBUFFER(w->contents) == current_buffer)
			...
			struct buffer *const old_buffer = current_buffer;
...
static void unshow_buffer(struct window *const w) {
	Lisp_Object buf = w->contents;
	struct buffer *const b = XBUFFER(buf);
	...
	if (
		...
		temp_set_point_both(
			b,
			clip_to_bounds(BUF_BEGV(b), marker_position(w->pointm), BUF_ZV(b)),
			clip_to_bounds(
				BUF_BEGV_BYTE(b), marker_byte_position(w->pointm), BUF_ZV_BYTE(b)
			)
		);
  ...
  if (setflag)
    {
      ...
      n->phys_cursor_type.kind = NO_CURSOR;
...
void set_window_buffer(
	Lisp_Object window, Lisp_Object buffer, bool run_hooks_p, bool keep_margins_p
) {
	struct window *w = XWINDOW (window);
...
//		Fset_marker(w->mark, BVAR(b, mark), buffer);
		// Set default window start.
		Fset_marker(w->start, make_fixnum(BUF_BEGV(b)), buffer);
		w->start_at_line_beg = false;
		w->force_start = false;
...
//  w->mark(w, Fmake_marker());
  ...
  w->phys_cursor_type.kind = NO_CURSOR;
  ...
  (Lisp_Object old, Lisp_Object pixel_size, Lisp_Object side, Lisp_Object normal_size)
{
	...
	// Based on set_window_buffer, but more efficiently sets window start.

	Lisp_Object buffer = r->contents;
	struct buffer *const b = XBUFFER(buffer);
	specpdl_ref count = SPECPDL_INDEX();
	...
	wset_update_mode_line(n);

	record_unwind_current_buffer();
	Fset_buffer(buffer);

	set_marker_both(n->pointm, buffer, PT, PT_BYTE);
	set_marker_both(n->old_pointm, buffer, PT, PT_BYTE);
//	Fset_marker(n->mark, BVAR(b, mark), buffer);
	// Set window start from reference window.
	Fset_marker(n->start, r->start, buffer);

	XMARKER(n->pointm)->insertion_type = XMARKER(n->old_pointm)->insertion_type =
		!NILP(Vwindow_point_insertion_type);

	if (!NILP(Vwindow_scroll_functions))
		run_hook_with_args_2(Qwindow_scroll_functions, new, Fmarker_position(n->start));
  ...
  if (!pos_visible_p (w, PT, &x, &y, &rtop, &rbot, &rowh, &vpos))
    {
	 ...
	 results for variable height lines.  */
      init_iterator (&it, w, PT, PT_BYTE, NULL, DEFAULT_FACE_ID, MODE_LINE_TYPE_NO);
...
window_scroll_line_based (Lisp_Object window, int n, bool whole, bool noerror)
{
  ...
  if (NILP (tem))
    {
      Fvertical_motion (make_fixnum (- (ht / 2)), window, Qnil, Qnil);
      startpos = PT;
      startbyte = PT_BYTE;
    }
  ...
  SET_PT_BOTH (startpos, startbyte);
  lose = n < 0 && PT == BEGV;
  Fvertical_motion (make_fixnum (n), window, Qnil, Qnil);
  pos = PT;
  pos_byte = PT_BYTE;
  bolp = Fbolp ();
  ...
  if (pos < ZV)
	  ...
	  && (whole || !EQ (Vscroll_preserve_screen_position, Qt)))
	{
	  ...
	  Fvertical_motion (original_pos, window, Qnil, Qnil);
	}
      ...
      else if (n > 0)
	{
	  ...
	  if (this_scroll_margin > 0)
	    {
	      SET_PT_BOTH (pos, pos_byte);
	      Fvertical_motion (make_fixnum (this_scroll_margin), window, Qnil, Qnil);
	      top_margin = PT;
	    }
	  else
	    top_margin = pos;
	  ...
	  if (top_margin <= marker_position (opoint_marker))
		...
		nlines = w->total_lines - this_scroll_margin - 1;
	      Fvertical_motion (Fcons (make_fixnum (window_scroll_preserve_hpos),
				       make_fixnum (nlines)), window, Qnil, Qnil);
	    }
	  else
	    SET_PT (top_margin);
	}
      else if (n < 0)
	{
	  ...
	  if (bottom_margin > marker_position (opoint_marker))
		...
		Fvertical_motion (make_fixnum (-1), window, Qnil, Qnil);
	    }
	}
    }
  else
  ...
  (Lisp_Object arg)
{
  ...
  if (start < BEGV || start > ZV)
    {
      ...
      Fvertical_motion (make_fixnum (- (height / 2)), window, Qnil, Qnil);
  ...
  return Fvertical_motion (arg, window, Qnil, Qnil);
}
...
static ptrdiff_t save_window_save(
	Lisp_Object window, struct Lisp_Vector *vector, ptrdiff_t i
) {
	struct saved_window *p;
	struct window *w;
		...
		if (BUFFERP (w->contents))
		{
			...
			if (EQ (window, selected_window)) {
				p->pointm = build_marker (XBUFFER (w->contents),
										  ...
										  BUF_PT_BYTE (XBUFFER (w->contents)));
				// Do the same with mark.
//				Lisp_Object mark = BVAR(XBUFFER(w->contents), mark);
//				p->mark = Fcopy_marker(mark, Qnil);
//				XMARKER(p->mark)->insertion_type = XMARKER(mark)->insertion_type;
			} else {
				p->pointm = Fcopy_marker (w->pointm, Qnil);
				// Do the same with mark.
//				p->mark = Fcopy_marker(w->mark, Qnil);
//				XMARKER(p->mark)->insertion_type = XMARKER(w->mark)->insertion_type;
			}
...
the buffer; `temp-buffer-show-hook' is not run unless this function runs it.  */);
  ...
  DEFVAR_BOOL ("mode-line-in-non-selected-windows", mode_line_in_non_selected_windows,
	       doc:
/* Non-nil means to use (mode/tab/header)-line-inactive faces in
non-selected windows.
If the minibuffer is active, the `minibuffer-selected-window' will serve
as a selected window in that case. */);
